...
email2:password2"></textarea><p class="help">Disembunyikan saat varian aktif karena stok diisi per varian.</p></div>
  ...
  <section id="supplierSettings" class="section">
    ...
    <div class="panel">
      <div class="sectionToolbar compactToolbar"><div><h2 class="sectionTitle">🤖 AI Flow Assistant</h2><p class="help">AI membantu menyusun Flow Order, Flow Cek Stok, Stock Regex, dan Regex Hasil dari langkah yang Anda tulis. AI hanya membuat draft; flow tidak dijalankan sampai Anda simpan sendiri.</p></div><span class="chip" id="supplierAiStatus">BELUM DIATUR</span></div>
      <form id="supplierAiConfigForm" class="form">
        <div class="row3"><div class="field"><label class="label">Base URL</label><input class="input" name="base_url" placeholder="https://api.x.ai/v1"></div><div class="field"><label class="label">Model</label><input class="input" name="model" placeholder="grok-4.5"></div><div class="field"><label class="label">Backend</label><select class="select" name="backend"><option value="chat_completions">Chat Completions</option><option value="responses">Responses API</option></select></div></div>
        <div class="field"><label class="label">API Key</label><input class="input" type="password" name="api_key" autocomplete="new-password" placeholder="Kosongkan jika tidak ingin mengganti API key yang sudah tersimpan"><p class="help" id="supplierAiKeyHint">API key disimpan terenkripsi server-side. Untuk xAI/Grok, gunakan API key dari xAI Console.</p></div>
        <div class="buttonRow"><button class="btn lime" type="submit">Simpan AI</button><button class="btn cyan" id="testSupplierAi" type="button">Tes Koneksi AI</button></div>
      </form>
    </div>
    <div class="panel">
...
(function(){
  ...
  function renderSupplierAi(){
    ...
    if(f){if(f.base_url)f.base_url.value=c.base_url||'https://api.x.ai/v1';if(f.model)f.model.value=c.model||'grok-4.5';if(f.backend)f.backend.value=c.backend||'chat_completions';if(f.api_key)f.api_key.value='';}
  ...
  async function loadSupplier(force){
    ...
    state.supplierAi=await apiSafe('supplier-ai-config',{configured:false,base_url:'https://api.x.ai/v1',model:'grok-4.5',backend:'chat_completions'});
    renderSupplierAi();
