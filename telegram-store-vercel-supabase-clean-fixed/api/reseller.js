...
@media(max-width:620px){.chart{height:260px;grid-template-columns:repeat(7,minmax(0,1fr));gap:4px;padding:8px 6px 10px}.bar{width:min(26px,68%)}.barLabel{font-size:9px;max-width:42px;line-height:1.1}}
    ...
    .promoTargetBox{border:var(--line);box-shadow:var(--soft);border-radius:var(--radius);background:#fff7c4;padding:14px}.promoTargetHead{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.promoTargetHead p{margin:4px 0 0}.compactSwitch{font-size:12px;white-space:nowrap}.compactSwitch .toggleTrack{width:48px;height:26px}.compactSwitch .toggleTrack:after{width:14px;height:14px}.compactSwitch input:checked+.toggleTrack:after{transform:translateX(21px)}.promoTargetBuilder{margin-top:12px}.promoTargetControls{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr) auto;gap:10px;align-items:end}.promoTargetControls .btn{min-height:47px}.promoTargetList{display:grid;gap:8px;margin-top:10px}.promoTargetItem{display:flex;align-items:center;justify-content:space-between;gap:10px;border:2px solid #000;border-radius:10px;background:#fff;padding:9px 10px}.promoTargetItem span{min-width:0;overflow-wrap:anywhere}.promoTargetItem button{border:2px solid #000;border-radius:8px;background:var(--red);color:#fff;padding:6px 9px;font-weight:900;cursor:pointer}.promoTargetEmpty{border:2px dashed #555;border-radius:10px;background:#fff;padding:12px;color:#555;font-size:12px}.rowBetween{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.voucherCard .rowBetween{align-items:flex-start}.voucherCard .actions{margin-top:0}.storeSubGrid{grid-template-columns:repeat(4,minmax(0,1fr))}.storeToolsInline{margin-top:18px;border-top:2px dashed #111;padding-top:16px}.profitPositive{color:#087f5b}.profitNegative{color:#c92a2a}.costSummary{border:2px solid #050505;border-radius:10px;background:#fff7d6;padding:8px 10px;font-size:12px;line-height:1.45}
    @media(max-width:760px){.forms,.dashboardGrid,.broadcastGrid{grid-template-columns:1fr}.storeSubGrid{grid-template-columns:repeat(2,minmax(0,1fr))}.promoSubBtn,.settingsSubBtn{flex-basis:178px;min-height:86px}.promoTargetControls{grid-template-columns:1fr 1fr}.promoTargetControls .btn{grid-column:1/-1}.sectionToolbar{align-items:flex-start}.sectionToolbar .btn{padding:10px 12px}}
    @media(max-width:620px){.wrap{padding:8px 10px 70px}.hero{padding:16px;margin-bottom:14px}.hero h1{font-size:27px}.statsGrid{gap:7px;margin-top:12px}.stat{min-height:58px;padding:8px}.navTiles{margin:10px 0 14px}.tile{min-width:88px;min-height:56px;padding:8px 7px;font-size:10px}.tile .ico{font-size:17px}.panel,.product,.miniCard{padding:12px}.sectionToolbar{flex-direction:column}.sectionToolbar .btn{width:100%}.row,.row3,.row4{grid-template-columns:1fr}.promoTargetHead{flex-direction:column}.compactSwitch{width:100%;justify-content:space-between;border-top:2px dashed #111;padding-top:10px}.promoTargetControls{grid-template-columns:1fr}.promoTargetControls .btn{grid-column:auto}.storeSubGrid{grid-template-columns:1fr 1fr}.voucherCard .rowBetween{display:block}.voucherCard .rowBetween .actions{margin-top:10px}.broadcastGrid .panel{margin-bottom:12px}}
...
email2:password2"></textarea><p class="help">Disembunyikan saat varian aktif karena stok diisi per varian.</p></div>
  ...
  <section id="promos" class="section">
    <div class="panel yellow"><h2 class="sectionTitle">Promo & Voucher</h2><p class="help">Pilih submenu untuk melihat daftar, membuat promo/voucher, atau mengatur Flash Sale.</p><div class="promoSubGrid"><button class="promoSubBtn active" type="button" data-promo-sub="list"><span class="ico">📋</span><b>Daftar</b><small>Lihat, edit, dan hapus promo maupun voucher.</small></button><button class="promoSubBtn" type="button" data-promo-sub="create"><span class="ico">➕</span><b>Buat Promo & Voucher</b><small>Tambah promo otomatis atau kode voucher.</small></button><button class="promoSubBtn" type="button" data-promo-sub="flash"><span class="ico">⚡</span><b>Flash Sale</b><small>Atur jadwal dan status Flash Sale Marketplace.</small></button></div></div>
  ...
  <section id="settings" class="section">
    <div class="panel settingsPanel">
      <h2 class="sectionTitle">Pengaturan</h2><p class="help">Pilih submenu. Semua submenu tersusun satu baris ke samping dan dapat digeser pada layar kecil.</p>
      <div class="settingsSubNav">
        <button class="settingsSubBtn active" type="button" data-settings-sub="store"><span class="ico">🏪</span><b>Pengaturan Toko</b><small>Nama, logo, CS, dan grup.</small></button>
        <button class="settingsSubBtn" type="button" data-settings-sub="banner"><span class="ico">🖼️</span><b>Banner Promosi</b><small>Daftar gambar dan kecepatan slide.</small></button>
        <button class="settingsSubBtn" type="button" data-settings-sub="start"><span class="ico">▶️</span><b>Media /start</b><small>Gambar, stiker, dan caption pembuka.</small></button>
      </div>
    </div>
    <div class="panel settingsPanel settingsSubPanel" id="settingsStorePanel" data-settings-panel="store">
      ...
      <div class="storeToolsInline">
        <h3 class="sectionTitle">Alat Toko</h3><p class="help">Lisensi, statistik, backup, dan maintenance sekarang berada di dalam Pengaturan Toko.</p>
        <div class="storeSubGrid">
          ...
          <button class="storeSubBtn" data-tab="deepStats" data-scroll-target="deepStats" type="button"><span class="ico">📈</span><b>Statistik Lengkap</b><small>Omzet, profit, stok, dan user</small></button>
    ...
    <div class="panel settingsPanel settingsSubPanel hidden" id="settingsBannerPanel" data-settings-panel="banner">
    ...
    <div class="panel settingsPanel settingsSubPanel hidden" id="settingsStartPanel" data-settings-panel="start">
...
(function(){
  ...
  function setSettingsSub(mode){ mode=mode||'store'; document.querySelectorAll('[data-settings-panel]').forEach(function(panel){ panel.classList.toggle('hidden',panel.dataset.settingsPanel!==mode); }); document.querySelectorAll('[data-settings-sub]').forEach(function(btn){ btn.classList.toggle('active',btn.dataset.settingsSub===mode); }); }
  ...
  function renderSettingsForm(){ var s=state.settings||{}; var store=document.getElementById('storeSettingsForm'); var banner=document.getElementById('bannerSettingsForm'); var start=document.getElementById('startMediaForm'); if(store){ ['store_name','logo_url','customer_service_link','group_link'].forEach(function(k){ if(store[k]) store[k].value=s[k]||''; }); } if(banner){ if(banner.banner_interval_seconds) banner.banner_interval_seconds.value=s.banner_interval_seconds||'5'; if(banner.store_description) banner.store_description.value=''; if(banner.banner_url) banner.banner_url.value=''; renderBannerRows(parseAdminBannerItems(s)); } if(start){ ['start_media_type','start_media_value','start_media_caption'].forEach(function(k){ if(start[k]) start[k].value=s[k]||(k==='start_media_type'?'none':''); }); } }
  ...
  function renderStats(){ var s=state.stats||{}; var daily=(state.analytics&&state.analytics.daily)||[]; var today=(state.analytics&&state.analytics.today_revenue!==undefined)?state.analytics.today_revenue:(daily.length?daily[daily.length-1].revenue:0); var items=[['Omset Hari Ini',rupiah(today)],['Profit Hari Ini',rupiah(s.profitToday||0)],['Profit Bulan Ini',rupiah(s.profitMonth||0)],['Total Profit',rupiah(s.profit||0)],['Order',s.orders||0],['Stok',s.stokTersedia||0]]; document.getElementById('stats').innerHTML=items.map(function(x){return '<div class="stat"><small>'+x[0]+'</small><b>'+x[1]+'</b></div>';}).join(''); }
  ...
  function renderProducts(){ var q=searchQuery(); var rows=state.products.filter(function(p){return productMatches(p,q);}); updateSearchCounter(); document.getElementById('productList').innerHTML=rows.map(function(p){ var varsArr=productVariants(p); var bulk=productBulkChips(p); var vars=varsArr.slice(0,3).map(function(v){return '<span class="chip '+(variantActive(v)?'purple':'red')+'">'+esc(v.name||v.nama)+' '+rupiah(v.price||v.harga||p.harga)+' • modal '+rupiah(v.cost_price||v.cost||0)+' • '+variantStock(v).length+' stok • '+(variantActive(v)?'ON':'OFF')+'</span>';}).join(''); return '<article class="product '+(p.active===false?'productOff':'')+'">'+
      ...
      '<div class="price">'+productDisplayPrice(p)+'</div><div class="costSummary">'+(varsArr.length?'Modal supplier diatur per varian. Nilainya dapat diubah lewat Edit.':'Modal default: <b>'+rupiah(p.cost_price||0)+'</b> · Margin normal '+rupiah(Number(p.harga||0)-Number(p.cost_price||0)))+'</div><div class="chips">'+bulk+vars+'</div>'+ 
  ...
  function openOrderCost(ref){
    ...
    function updatePreview(){ var cost=Math.max(0,Number((input&&input.value)||0)); var profit=net-cost; if(preview) preview.innerHTML='Modal per item (perkiraan): <b>'+rupiah(Math.round(cost/qty))+'</b> · Profit kotor: <b class="'+(profit<0?'profitNegative':'profitPositive')+'">'+rupiah(profit)+'</b>'; }
  ...
  function openOrderProducts(ref){
    ...
    var known=orderCostKnown(o);
    var profit=orderProfit(o);
    var costText=known?rupiah(o.cost_total||0):'Belum diatur';
    var profitText=known?rupiah(profit):'Menunggu modal';
    openModal('Detail Penjualan', '<div class="detailGrid"><div class="detailItem"><b>Invoice</b><br>'+esc(displayRef(o.order_ref||'-'))+'</div><div class="detailItem"><b>User</b><br>'+user+'</div><div class="detailItem"><b>Produk</b><br>'+esc(o.product_name||'-')+(o.variant_name?' - '+esc(o.variant_name):'')+'</div><div class="detailItem"><b>Total Dibayar</b><br>'+rupiah(o.total_price)+'</div><div class="detailItem"><b>Fee Pembayaran</b><br>'+rupiah(o.payment_fee||0)+'</div><div class="detailItem"><b>Omzet Bersih</b><br>'+rupiah(orderNetSales(o))+'</div><div class="detailItem"><b>Modal Supplier</b><br>'+costText+'</div><div class="detailItem"><b>Profit Kotor</b><br><span class="'+(profit<0?'profitNegative':'profitPositive')+'">'+profitText+'</span></div></div><div class="field" style="margin-top:12px"><label class="label">Produk yang diterima pembeli</label><textarea class="textarea tall" readonly>'+esc(text)+'</textarea></div><button class="btn yellow" type="button" id="editOrderCostFromDetail">Atur / Koreksi Modal Checkout</button>');
    var edit=document.getElementById('editOrderCostFromDetail'); if(edit) edit.onclick=function(){ openOrderCost(o.order_ref); };
  }
  function orderMatches(o,q){ return textMatch([o.order_ref,o.product_name,o.variant_name,o.username,o.telegram_id,o.total_price,o.quantity,o.created_at,o.cost_total,o.profit_amount,orderProductText(o)],q); }
  function renderOrders(){
    ...
    document.getElementById('orderList').innerHTML=rows.map(function(o){
      ...
      var known=orderCostKnown(o);
      var profit=orderProfit(o);
      var costLine=known?rupiah(o.cost_total||0):'<span class="chip red">Belum diatur</span>';
      var profitLine=known?('<b class="'+(profit<0?'profitNegative':'profitPositive')+'">'+rupiah(profit)+'</b>'):'<span class="chip yellow">Menunggu modal</span>';
      return '<article class="orderCard"><div class="orderRef">'+ref+'</div><b class="orderTitle">'+name+'</b><span class="statusDone">COMPLETED</span><div class="orderMeta">×'+esc(o.quantity||1)+' · Total <b>'+rupiah(o.total_price)+'</b><br>Fee: '+rupiah(o.payment_fee||0)+' · Omzet bersih: <b>'+rupiah(orderNetSales(o))+'</b><br>Modal supplier: '+costLine+'<br>Profit kotor: '+profitLine+'<br>👤 '+user+'<br>🗓 '+new Date(o.created_at).toLocaleString('id-ID')+'</div><div class="actions"><button class="btn small purple" type="button" data-order-products="'+esc(rawRef)+'">Detail</button><button class="btn small yellow" type="button" data-order-cost="'+esc(rawRef)+'">Atur Modal</button></div></article>';
  ...
  function promoUnifiedReset(){
    ...
    f.reset(); f.current_code.value=''; setPromoTargets([]); var check=document.getElementById('promoFlashSale'); if(check)check.checked=false; updatePromoFlashSaleControl();
  ...
  function renderDeepStats(){
    ...
    var box=document.getElementById('deepStatsBox'); if(box){ var rows=[['Omset Hari Ini',rupiah(d.revenue_today)],['Profit Kotor Hari Ini',rupiah(d.profit_today)],['Omset Bulan Ini',rupiah(d.revenue_month)],['Profit Kotor Bulan Ini',rupiah(d.profit_month)],['Total Omset Semua Waktu',rupiah(d.revenue_total)],['Total Modal Tercatat',rupiah(d.cost_total)],['Total Profit Kotor',rupiah(d.profit_total)],['Rata-rata Nilai Order',rupiah(d.average_order_value)],['Total Item Terjual',d.quantity_sold||0],['Estimasi Checkout Berhasil',(d.conversion_rate||0)+'%'],['Promo Otomatis Aktif',d.active_promos||0],['Pending Order Aktif',d.pending_orders||0]]; box.innerHTML=rows.map(function(r){return '<div class="detailItem"><b>'+esc(r[0])+'</b><br><span style="font-size:22px">'+esc(r[1])+'</span></div>';}).join(''); }
  ...
  document.querySelectorAll('[data-settings-sub]').forEach(function(btn){btn.onclick=function(){ setSettingsSub(btn.dataset.settingsSub||'store'); };});
  ...
  var promoKind=document.querySelector('#promoUnifiedForm [name="promo_kind"]'); if(promoKind) promoKind.onchange=updatePromoFlashSaleControl; updatePromoFlashSaleControl(); setPromoSub('list'); setSettingsSub('store');
