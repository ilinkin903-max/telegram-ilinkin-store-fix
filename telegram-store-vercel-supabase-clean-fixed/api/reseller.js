...
(function(){
  ...
  var state = { stats:{}, products:[], orders:[], users:[], vouchers:[], polls:[], settings:{}, analytics:{}, maintenance:{} };
  ...
  async function load(){ try{ var all=await Promise.all([api('stats'),api('products'),api('orders'),api('users'),api('vouchers'),api('settings'),api('analytics'),api('polls'),api('maintenance-stats')]); state.stats=all[0].data; state.products=all[1].data; state.orders=all[2].data; state.users=all[3].data; state.vouchers=all[4].data; state.settings=all[5].data; state.analytics=all[6].data; state.polls=all[7].data||[]; state.maintenance=all[8].data||{}; renderHeader(); renderStats(); renderCharts(); renderProducts(); renderOrders(); renderUsers(); renderVouchers(); renderPolls(); renderMaintenance(); }catch(e){ toast(e.message,true); renderStats(); renderProducts(); renderMaintenance(); } }
