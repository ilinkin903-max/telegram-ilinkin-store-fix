...
email2:password2"></textarea><p class="help">Disembunyikan saat varian aktif karena stok diisi per varian.</p></div>
  ...
  <section id="promos" class="section">
    <div class="panel yellow"><h2 class="sectionTitle">Promo & Voucher</h2><p class="help">Atur voucher, promo otomatis, sekaligus Flash Sale Marketplace dari satu menu.</p><div class="promoSubGrid"><button class="promoSubBtn active" type="button" data-promo-sub="list"><span class="ico">📋</span><b>Daftar Promo & Voucher</b><small>Lihat, edit, dan hapus voucher/promo.</small></button><button class="promoSubBtn" type="button" data-promo-sub="create"><span class="ico">➕</span><b>Buat Promo / Voucher</b><small>Tambah promo otomatis atau kode voucher.</small></button></div></div>
    <div class="panel flashSaleAdminPanel">
      <h2 class="sectionTitle">⚡ Flash Sale Marketplace</h2>
      <p class="help">Produk masuk Flash Sale melalui pilihan pada <b>Promo Otomatis</b>. Jumlah terjual dihitung hanya dari transaksi selama periode Flash Sale.</p>
    ...
    <div class="panel lime hidden" id="promoCreatePanel"><h2 class="sectionTitle">Buat / Edit Promo & Voucher</h2>
      <form id="promoUnifiedForm" class="form">
        ...
        <div class="switchBox" id="promoFlashSaleBox">
          ...
          <p class="help">Hanya tersedia untuk Promo Otomatis. Jika promo menargetkan varian tertentu, Flash Sale akan menampilkan nama varian tersebut tepat di bawah nama produk.</p>
        </div>
...
(function(){
  ...
  function renderSettingsForm(){ var s=state.settings||{}; var f=document.getElementById('settingsForm'); if(!f) return; ['store_name','logo_url','customer_service_link','group_link','start_media_type','start_media_value','start_media_caption','banner_interval_seconds'].forEach(function(k){ if(!f[k]) return; var fallback=(k==='start_media_type'?'none':(k==='banner_interval_seconds'?'5':'')); f[k].value = s[k] || fallback; }); if(f.store_description) f.store_description.value=''; if(f.banner_url) f.banner_url.value=''; renderBannerRows(parseAdminBannerItems(s)); }
  ...
  document.getElementById('settingsForm').onsubmit=async function(e){e.preventDefault(); var d=formDataRaw(e.target); var banners=collectBannerRows(); d.store_description=''; d.banner_items=JSON.stringify(banners); d.banner_urls=banners.map(function(x){return x.url;}).join('\n'); d.banner_url=banners.length?banners[0].url:''; d.banner_interval_seconds=Math.max(3,Math.min(15,Number(d.banner_interval_seconds||5))); await post('save-settings',d);};
  ...
  var promoKind=document.querySelector('#promoUnifiedForm [name="promo_kind"]'); if(promoKind) promoKind.onchange=updatePromoFlashSaleControl; updatePromoFlashSaleControl();
