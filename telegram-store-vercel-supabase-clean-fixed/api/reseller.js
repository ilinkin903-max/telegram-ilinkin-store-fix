...
email2:password2"></textarea><p class="help">Disembunyikan saat varian aktif karena stok diisi per varian.</p></div>
        ...
        <div class="switchBox">
          ...
          <div id="addVariantBuilder" class="variantBuilder">
            <div class="variantMainCompact">Mode varian aktif: harga, deskripsi, SnK, dan grosir diatur per varian. Untuk stok, setiap varian bisa memilih STOK TERPISAH atau STOK BERSAMA.</div>
            <div id="addSharedStockWrap" class="field sharedStockField"><label class="label">Stok Produk Bersama</label><textarea class="textarea tall" id="addSharedStock" placeholder="Satu stok per baris
akun1:password1
akun2:password2"></textarea><p class="help">Dipakai hanya oleh varian yang memilih <b>STOK BERSAMA</b>. Semua varian Shared mengambil dari pool yang sama.</p></div>
  ...
  <section id="storeSettings" class="section">
    ...
    <div class="panel settingsPanel">
      ...
      <form id="storeSettingsForm" class="form">
        ...
        <div class="field"><label class="label">URL / Username Bot Nokos</label><input class="input" name="nokos_link" maxlength="500" placeholder="@BotNokos, BotNokos, t.me/BotNokos, atau https://t.me/BotNokos"><p class="help">Tombol <b>Nokos</b> tampil di sebelah <b>Daftar Produk</b> pada menu awal. Masukkan username Telegram atau URL tujuan. Kosongkan untuk menyembunyikan tombol.</p></div>
        <div class="row3">
...
<style>.sharedStockField.hidden{display:none}.sharedStockField{margin:8px 0;padding:12px;border:2px solid #111;border-radius:12px;background:#e5fbff}.sharedStockField .help{margin-top:6px}</style>
<script>
(function(){
  ...
  function mergeVariantStockArray(product, mode){
    ...
    return existing.map(function(v,i){
      ...
      var stock=isSupplierVariant ? (isWorkflowVariant ? variantStock(v) : []) : (variantUsesSharedStock(v) ? variantStock(v) : (mode==='append' ? variantStock(v).concat(input) : input));
      return {
        name:v.name||v.nama||'',
  ...
  function addVariantRow(data){
    ...
    div.innerHTML='<div class="addVariantCardTitle"><b>Varian '+n+'</b><div><label class="miniSwitch"><input type="checkbox" data-vfield="active" '+(data.active===false?'':'checked')+'><span>ON</span></label> <button class="btn small red" type="button" data-remove-variant>Hapus</button></div></div>'+ 
      ...
      '<div class="field"><label class="label">Sumber Stok Varian</label><select class="select" data-vfield="stock_mode"><option value="separate" '+(String(data.stock_mode||'separate')==='separate'?'selected':'')+'>STOK TERPISAH · khusus varian ini</option><option value="shared" '+(String(data.stock_mode||'')==='shared'?'selected':'')+'>STOK BERSAMA · gunakan Stok Produk utama</option></select><p class="help">Shared = semua varian memakai satu pool Stok Produk Bersama. Separate = stok khusus varian ini.</p></div><div class="row"><div class="field" data-local-variant-stock><label class="label">Stok Varian Terpisah</label><textarea class="textarea" data-vfield="stock" placeholder="Contoh, pisahkan koma atau baris baru:\nakun1,akun2,akun3">'+esc(data.stock||'')+'</textarea><p class="help">Field ini hanya dipakai ketika sumber stok adalah STOK TERPISAH.</p></div><div class="field"><label class="label">Harga Grosir Varian</label><textarea class="textarea" data-vfield="bulk" placeholder="Contoh:\n5:9000,10:8000">'+esc(data.bulk||'')+'</textarea></div></div>'+
    ...
    remove.onclick=function(){ div.remove(); refreshVariantTitles(); refreshAddSharedStockVisibility(); };
    var source=div.querySelector('[data-vfield="stock_mode"]');
    if(source) source.onchange=function(){ syncAddVariantStockSource(div); refreshAddSharedStockVisibility(); };
    syncAddVariantStockSource(div);
    refreshAddSharedStockVisibility();
  }
  ...
  function syncAddVariantStockSource(card){
    if(!card) return;
    var source=card.querySelector('[data-vfield="stock_mode"]');
    var field=card.querySelector('[data-local-variant-stock]');
    var shared=source && String(source.value||'')==='shared';
    if(field) field.classList.toggle('hidden', shared);
  }
  function refreshAddSharedStockVisibility(){
    ...
    document.querySelectorAll('#addVariantCards .addVariantCard').forEach(function(card){
      syncAddVariantStockSource(card);
      var el=card.querySelector('[data-vfield="stock_mode"]');
      if(el && String(el.value||'')==='shared') shared=true;
    });
  ...
  function openWorkflowStepEdit(stepId){
    ...
    var knownButtons=[]; var knownButtonSeen={};
    [step.source_message_snapshot||{},step.response_snapshot||{}].concat(Array.isArray(step.response_snapshots)?step.response_snapshots:[]).forEach(function(snap){(snap&&snap.buttons||[]).forEach(function(b){var text=String(b&&b.text||'').trim();if(!text)return;var key=String(b.kind||'')+'|'+text;if(knownButtonSeen[key])return;knownButtonSeen[key]=true;knownButtons.push({text:text,kind:String(b.kind||'')});});});
    var buttonRole=String(step.button_role||'static')==='quantity'?'quantity':'static';
    var buttonPicker='';
    if(type==='button'){
      buttonPicker='<div id="stepEditButtonOptions" class="field"><label class="label">🔘 Tombol pada Pesan Bot</label><p class="help">Daftar tombol yang terekam pada pesan ini tetap disimpan saat workflow disalin. Tekan salah satu tombol di bawah untuk mengganti tombol yang harus diklik; Anda tidak perlu mengetik ulang persis.</p><div class="workflowButtonPicker">'+(knownButtons.length?knownButtons.map(function(b){var selected=String(b.text)===String(step.action_value||'');return b.kind==='url'?'<button class="btn small" type="button" disabled>🔗 '+esc(b.text)+'</button>':'<button class="btn small '+(selected?'selected':'')+'" type="button" data-workflow-pick-button="'+encodeURIComponent(b.text)+'">'+(selected?'✓ ':'🔘 ')+esc(b.text)+'</button>';}).join(''):'<span class="subtle">Daftar tombol belum tersedia pada rekaman step ini. Rekam ulang/Refresh pesan supplier untuk menangkap keyboard.</span>')+'</div></div>';
      buttonPicker+='<div id="stepEditButtonRoleBox" class="field"><label class="label">Peran Tombol</label><select class="select" name="button_role"><option value="static" '+(buttonRole==='static'?'selected':'')+'>Tombol tetap</option><option value="quantity" '+(buttonRole==='quantity'?'selected':'')+'>🔢 Jumlah Item Otomatis sesuai quantity</option></select><p class="help">Pilih <b>Jumlah Item Otomatis</b> untuk menu seperti <b>1 / 2 / 3 / 4 / 5</b>. Saat customer membeli 3, bot otomatis menekan tombol 3; saat membeli 1, otomatis menekan tombol 1. Saat direkam sistem juga mencoba mendeteksi menu angka seperti ini secara otomatis.</p></div>';
    }
    var extraction='';
  ...
  function renderWorkflow(){
    ...
    var detailSteps=(state.workflowDetail&&state.workflowDetail.steps)||[]; var recorderLastStep=detailSteps[detailSteps.length-1]||{}; var snap=current?(current.last_message_snapshot||{}):{}; var candidates=current&&Array.isArray(current.recent_message_snapshots)?current.recent_message_snapshots.slice():[]; if(!candidates.length&&snap&&snap.id)candidates=[snap]; var sourceSnap=recorderLastStep&&recorderLastStep.source_message_snapshot||{}; if(sourceSnap&&sourceSnap.id&&!candidates.some(function(x){return Number(x&&x.id||0)===Number(sourceSnap.id||0);})){candidates.push(Object.assign({},sourceSnap,{_workflow_source:true,currently_visible:sourceSnap.currently_visible!==false}));} var selectedMessageId=Number(current&&current.last_message_id||0); var clickedByMessage={}; detailSteps.forEach(function(st){ if(String(st.action_type||'').toLowerCase()!=='button')return; var ids=[Number(st.source_message_snapshot&&st.source_message_snapshot.id||0),Number(st.response_snapshot&&st.response_snapshot.id||0)].filter(Boolean); ids.forEach(function(mid){clickedByMessage[mid]=clickedByMessage[mid]||{};var raw=String(st.action_value||'').trim().toLowerCase().replace(/^[\s•○●◦◉◎☑✅✔✓□■▪▫·]+/u,'').replace(/\(\s*[-+]?\d[\d.,]*\s*\)/g,'(#)').replace(/\s+/g,' ').trim();clickedByMessage[mid][raw]=st;}); }); var msg=document.getElementById('workflowLastMessage');
    if(msg){
    ...
    document.querySelectorAll('[data-workflow-restart]').forEach(function(btn){btn.onclick=async function(){if(!confirm('Workflow akan diulang dari /start menggunakan invoice/penjualan yang SAMA. Tidak dibuat penjualan baru di marketplace. Pastikan order supplier sebelumnya benar-benar belum berhasil agar tidak terjadi pembelian supplier ganda. Lanjutkan?'))return; try{await api('workflow-retry-order',{order_ref:btn.dataset.workflowRestart,force_restart:true}); state.workflowLoaded=false; await loadWorkflow(true); toast('Workflow dimulai ulang.');}catch(e){toast(e.message,true);}};});
  }
  ...
  function wireEditVariantRemoveButtons(){ document.querySelectorAll('[data-remove-edit-variant]').forEach(function(btn){ btn.onclick=function(){ var card=btn.closest('[data-edit-variant-card]'); if(card) card.remove(); refreshEditVariantTitles(); refreshEditSharedStockVisibility(window.__editingProduct||null); }; }); }
  ...
  function openEditProduct(code){
    ...
    window.__editingProduct=p; if(!p) return;
  ...
  document.getElementById('addForm').onsubmit=async function(e){
    ...
    if(t&&t.checked){
      if(variants.length){
        ...
        var anyShared=variants.some(function(v){ return v.stock_mode==='shared'; });
        if(anyShared){
          var sharedInput=document.getElementById('addSharedStock');
          payload.stock_text=sharedInput?cleanListText(sharedInput.value).join('\n'):'';
        } else {
          payload.stock_text='';
        }
      }
  ...
  var workflowDelete=document.getElementById('workflowDelete'); if(workflowDelete) workflowDelete.onclick=async function(){var id=workflowCurrentId(); if(!id)return toast('Pilih workflow dulu.',true); if(!confirm('Hapus workflow ini? Produk tidak akan lagi terhubung ke workflow tersebut. Riwayat run/step workflow ini juga akan dihapus, tetapi transaksi/order marketplace tidak dihapus.'))return; try{await api('workflow-delete',{workflow_id:id}); state.workflowLoaded=false; state.workflowDetail=null; await load(); await loadWorkflow(true); toast('Workflow dihapus.');}catch(e){toast(e.message,true);}};
