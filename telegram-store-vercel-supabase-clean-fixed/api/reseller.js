...
<div class="wrap">
  <header class="hero">
    ...
    <div class="storeline byline"><span>By iLink</span><button data-tab="settings" type="button">Ubah</button></div>
...
10|9000"></textarea><p class="help">Dipakai untuk produk tanpa varian, atau jika varian belum punya harga grosir sendiri.</p></div>
          <div class="field"><label class="label">Stok Default / Non-Varian</label><textarea class="textarea" name="stock_text" placeholder="Contoh: satu baris satu stok
email1:password1
email2:password2"></textarea><p class="help">Jika varian aktif, stok bisa dimasukkan di masing-masing varian.</p></div>
        </div>
        <div class="switchBox">
  ...
  <section id="vouchers" class="section">
    <div class="panel"><h2 class="sectionTitle">Voucher</h2><p class="help">Kelola voucher lewat popup agar lebih mudah. Kode voucher dapat berlaku untuk semua produk atau kode produk tertentu.</p><button class="btn lime" type="button" id="openAddVoucher">+ Tambah Voucher</button></div>
...
(function(){
  ...
  function renderProducts(){ var q=(document.getElementById('search').value||'').toLowerCase(); var rows=state.products.filter(function(p){return productMatches(p,q);}); document.getElementById('productCounter').textContent=rows.length+' / '+state.products.length+' produk'; document.getElementById('productList').innerHTML=rows.map(function(p){ var bulk=(p.bulk_prices||[]).slice(0,2).map(function(b){return '<span class="chip yellow">'+(b.min_qty||b.qty)+'+ '+rupiah(b.price||b.harga)+'</span>';}).join(''); var vars=(p.variants||[]).slice(0,3).map(function(v){return '<span class="chip purple">'+esc(v.name||v.nama)+' '+rupiah(v.price||v.harga||p.harga)+' • '+variantStock(v).length+' stok</span>';}).join(''); return '<article class="product">'+
      '<div class="productTop">'+productMediaHtml(p)+'<div><h3>'+esc(p.nama)+'</h3><div class="subtle">'+esc(p.category||'Produk')+' - STOK '+stockCount(p)+'</div></div><span class="approved">APPROVED</span></div>'+ 
  ...
  function editFormHtml(p){ return '<form id="modalEditForm" class="form"><input type="hidden" name="kode" value="'+esc(p.kode)+'"><div class="row3"><input class="input" name="nama" placeholder="Nama produk" value="'+esc(p.nama)+'"><input class="input" name="kode_baru" placeholder="Kode baru" value="'+esc(p.kode)+'"><input class="input" name="harga" type="number" placeholder="Harga dasar" value="'+esc(p.harga)+'"></div><div class="row"><input class="input" name="category" placeholder="Kategori" value="'+esc(p.category||'')+'"><input class="input" name="image_url" placeholder="URL gambar" value="'+esc(p.image_url||'')+'"></div><div class="row"><textarea class="textarea" name="deskripsi" placeholder="Deskripsi default produk">'+esc(p.deskripsi||'')+'</textarea><textarea class="textarea" name="snk" placeholder="SnK default produk">'+esc(p.snk||'')+'</textarea></div><div class="row"><div><label class="label">Harga grosir default</label><textarea class="textarea" name="bulk_text">'+esc(bulkToText(p.bulk_prices||[]))+'</textarea></div><div><label class="label">Edit Varian Produk</label><button class="btn purple" type="button" id="openVariantEditor">Edit Varian / Harga / Stok</button><p class="help">Deskripsi dan SnK varian diedit dari tombol ini agar lebih mudah dipahami.</p></div></div><label class="label">Stok default / non-varian</label><textarea class="textarea tall" name="stock_text">'+esc((p.data||[]).join('\n'))+'</textarea><button class="btn cyan" type="submit">Simpan Perubahan</button></form>'; }
  function openEditProduct(code){ var p=findProduct(code); if(!p) return; openModal('Edit Produk - '+p.nama, editFormHtml(p)); var variantBtn=document.getElementById('openVariantEditor'); if(variantBtn) variantBtn.onclick=function(){ openVariantProduct(code); }; document.getElementById('modalEditForm').onsubmit=async function(e){ e.preventDefault(); var d=formData(e.target); d.current_code=d.kode; if(d.kode_baru) d.kode=d.kode_baru; delete d.kode_baru; await post('edit-product-full',d); closeModal(); }; }
  function openVariantProduct(code){ var p=findProduct(code); if(!p) return; var html='<form id="modalVariantForm" class="form"><input type="hidden" name="kode" value="'+esc(p.kode)+'"><h3>Edit Varian Produk</h3><p class="help">Satu baris = satu varian. Format: Nama Varian | Harga | Kode | Stok | Harga Grosir | Deskripsi Varian | SnK Varian</p>'+variantHelp()+'<textarea class="textarea tall" name="variants_text" placeholder="1 Bulan|10000|BULAN1|akun1,akun2|5:9000,10:8000|Deskripsi khusus varian|SnK khusus varian">'+esc(variantsToText(p.variants||[]))+'</textarea><button class="btn purple" type="submit">Simpan Varian</button></form>'; openModal('Edit Varian Produk - '+p.nama, html); document.getElementById('modalVariantForm').onsubmit=async function(e){ e.preventDefault(); await post('edit-product-full',formData(e.target)); closeModal(); }; }
  function openStockProduct(code){ var p=findProduct(code); if(!p) return; var html='<form id="modalStockForm" class="form"><input type="hidden" name="kode" value="'+esc(p.kode)+'"><p class="help">Stok varian diedit lewat format varian. Stok default dipakai untuk produk tanpa varian.</p><label class="label">Stok default / non-varian</label><textarea class="textarea tall" name="stock_text">'+esc((p.data||[]).join('\n'))+'</textarea><label class="label">Varian + stok + grosir</label><textarea class="textarea tall" name="variants_text">'+esc(variantsToText(p.variants||[]))+'</textarea><button class="btn lime" type="submit">Simpan Stok</button></form>'; openModal('Edit Stok - '+p.nama, html); document.getElementById('modalStockForm').onsubmit=async function(e){ e.preventDefault(); await post('edit-product-full',formData(e.target)); closeModal(); }; }
  function openManageProduct(code){ var p=findProduct(code); if(!p) return; var variants=(p.variants||[]).map(function(v){ return '<div class="detailItem"><b>'+esc(v.name)+'</b><br>Harga: '+rupiah(v.price||p.harga)+'<br>Stok: '+variantStock(v).length+'<br>Grosir: '+esc(bulkToText(v.bulk_prices||[]).replace(/\n/g,', ')||'-')+'</div>'; }).join('')||'<div class="detailItem">Tidak ada varian. Stok default: '+((p.data||[]).length)+'</div>'; openModal('Kelola - '+p.nama,'<div class="detailGrid">'+variants+'</div><br><button class="btn cyan" id="manageEdit">Edit Produk</button> <button class="btn lime" id="manageStock">Edit Stok</button>'); document.getElementById('manageEdit').onclick=function(){openEditProduct(code);}; document.getElementById('manageStock').onclick=function(){openStockProduct(code);}; }
  ...
  function wireProductButtons(){ document.querySelectorAll('[data-edit-product]').forEach(function(btn){btn.onclick=function(){openEditProduct(btn.dataset.editProduct);};}); document.querySelectorAll('[data-manage-product]').forEach(function(btn){btn.onclick=function(){openManageProduct(btn.dataset.manageProduct);};}); document.querySelectorAll('[data-stock-product]').forEach(function(btn){btn.onclick=function(){openStockProduct(btn.dataset.stockProduct);};}); document.querySelectorAll('[data-delete-product]').forEach(function(btn){btn.onclick=function(){openDeleteProduct(btn.dataset.deleteProduct);};}); }
  ...
  function voucherFormHtml(v){ v=v||{}; var target=(v.products&&v.products.length)?v.products.join(','):'semua'; return '<form id="modalVoucherForm" class="form"><input type="hidden" name="current_code" value="'+esc(v.code||'')+'"><input class="input" name="kode" placeholder="Contoh: DISKON10" value="'+esc(v.code||'')+'" required><input class="input" name="produk" placeholder="Contoh: semua atau CANVA1B,NETFLIX" value="'+esc(target)+'"><div class="row"><input class="input" name="potongan" type="number" placeholder="Contoh: 10000" value="'+esc(v.discount||'')+'" required><input class="input" name="limit" type="number" placeholder="Contoh: 20" value="'+esc(v.usage_limit||'')+'" required></div><div class="row"><select class="select" name="active"><option value="true" '+(v.active===false?'':'selected')+'>Aktif</option><option value="false" '+(v.active===false?'selected':'')+'>Nonaktif</option></select><input class="input" name="expires_at" placeholder="Opsional: 2026-07-30T23:59:00+07:00" value="'+esc(v.expires_at||'')+'"></div><textarea class="textarea" name="description" placeholder="Contoh: Voucher promo khusus member lama.">'+esc(v.description||'')+'</textarea><button class="btn lime" type="submit">Simpan Voucher</button></form>'; }
