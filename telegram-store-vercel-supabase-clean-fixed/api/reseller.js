...
@media(max-width:900px){#modalEditForm{padding-bottom:76px}.editSaveDock{position:fixed;left:12px;right:12px;bottom:12px;z-index:145;max-width:860px;margin:0 auto;padding:8px 9px;box-shadow:5px 5px 0 #000}.editSaveDock>div{display:none}.editSaveDock .btn{width:100%;min-height:46px}}
    ...
    @media(max-width:900px){
      .editSaveDock{bottom:6px!important}
      #modalEditForm{padding-bottom:78px}
    }
...
email2:password2"></textarea><p class="help">Disembunyikan saat varian aktif karena stok diisi per varian.</p></div>
  ...
  <section id="settings" class="section">
    <div class="panel settingsPanel settingsMenuPanel">
      ...
      <div class="settingsSubNav">
        ...
        <button class="settingsSubBtn" data-tab="supplierSettings" type="button"><span class="ico">🔄</span><b>Supplier / Reseller</b><small>Hubungkan ProdSeller API dan bot supplier Telegram Userbot dalam satu tempat.</small></button>
  ...
  <section id="storeSettings" class="section">
    ...
    <div class="panel settingsPanel">
      ...
      <form id="storeSettingsForm" class="form">
        ...
        <div class="row"><div class="field"><label class="label">Total User di Menu /start</label><select class="select" name="bot_show_total_users"><option value="true">Tampilkan</option><option value="false">Sembunyikan</option></select><p class="help">Jika disembunyikan, baris Total User tidak ditampilkan pada menu awal bot.</p></div><div class="field"><label class="label">Wajib Join Channel</label><select class="select" name="join_channel_required"><option value="false">Tidak wajib</option><option value="true">Wajib sebelum menggunakan bot</option></select><p class="help">Jika aktif, user harus menjadi member channel sebelum /start, daftar produk, saldo, dan menu bot dapat digunakan.</p></div></div>
        <div class="row"><div class="field"><label class="label">Username / ID Channel Wajib Join</label><input class="input" name="join_channel_id" placeholder="@channelkamu atau -1001234567890"><p class="help">Untuk pengecekan member yang stabil, jadikan bot sebagai admin di channel. Untuk channel publik cukup isi @username.</p></div><div class="field"><label class="label">Link Tombol Join Channel</label><input class="input" name="join_channel_link" placeholder="https://t.me/channelkamu"><p class="help">Link ini ditampilkan ke user yang belum join. Untuk channel private, isi invite link di sini dan ID channel numerik pada kolom sebelah.</p></div></div>
  ...
  <section id="supplierSettings" class="section">
    ...
    <div class="panel">
      <div class="sectionToolbar compactToolbar"><div><h2 class="sectionTitle">🤖 AI Flow Assistant · Google Gemini</h2><p class="help">Gemini membantu menyusun Flow Order, Flow Cek Stok, Stock Regex, dan Regex Hasil dari langkah yang Anda tulis. AI hanya membuat draft; flow tidak dijalankan sampai Anda simpan sendiri.</p></div><span class="chip" id="supplierAiStatus">BELUM DIATUR</span></div>
      <form id="supplierAiConfigForm" class="form">
        <input type="hidden" name="base_url" value="https://generativelanguage.googleapis.com/v1beta/openai">
        <input type="hidden" name="backend" value="chat_completions">
        <div class="row2"><div class="field"><label class="label">Provider</label><input class="input" value="Google Gemini" readonly><p class="help">Base URL otomatis: generativelanguage.googleapis.com/v1beta/openai</p></div><div class="field"><label class="label">Model Gemini</label><input class="input" name="model" list="geminiModelList" placeholder="gemini-3.7-flash"><datalist id="geminiModelList"><option value="gemini-3.7-flash"><option value="gemini-3.6-flash"><option value="gemini-3.5-flash"><option value="gemini-2.5-flash"><option value="gemini-2.5-flash-lite"></datalist><p class="help" id="supplierAiModelHint">Klik <b>Muat Model dari Google</b> agar daftar model mengikuti API key/project Anda.</p></div></div>
        <div class="field"><label class="label">Gemini API Key</label><input class="input" type="password" name="api_key" autocomplete="new-password" placeholder="Masukkan API key dari Google AI Studio"><p class="help" id="supplierAiKeyHint">API key disimpan terenkripsi server-side dan tidak ditampilkan kembali.</p></div>
        <div class="buttonRow"><button class="btn lime" type="submit">Simpan Gemini</button><button class="btn cyan" id="loadSupplierAiModels" type="button">Muat Model dari Google</button><button class="btn cyan" id="testSupplierAi" type="button">Tes Koneksi Gemini</button><a class="btn yellow" href="https://aistudio.google.com/apikey" target="_blank" rel="noopener">Buat API Key Google</a></div>
      </form>
    </div>
    <div class="panel">
      <div class="sectionToolbar compactToolbar"><div><h2 class="sectionTitle">Telegram Userbot Supplier</h2><p class="help">Tambahkan bot supplier seperti @Vinnstore_bot. Vercel membuka session Telegram hanya saat cek stok atau order, lalu disconnect. Saldo supplier diisi manual di dashboard dan otomatis berkurang setelah order supplier berhasil.</p></div><div class="buttonRow"><a class="btn cyan" href="/setup-userbot.html" target="_blank" rel="noopener">Buat TG_STRING_SESSION</a><span class="chip">v82.1 · VERCEL ON-DEMAND</span></div></div>
      <form id="telegramSupplierConnectorForm" class="form">
        <input type="hidden" name="id">
        <div class="row3"><div class="field"><label class="label">Kode Supplier</label><input class="input" name="code" placeholder="vinnstore" required></div><div class="field"><label class="label">Nama Supplier</label><input class="input" name="name" placeholder="Vinnstore" required></div><div class="field"><label class="label">Bot Supplier</label><input class="input" name="bot_username" placeholder="@Vinnstore_bot" required></div></div>
        <div class="row3"><div class="field"><label class="label">Status</label><select class="select" name="enabled"><option value="false">Nonaktif dulu</option><option value="true">Aktif</option></select></div><div class="field"><label class="label">Saldo Supplier Manual</label><input class="input" type="number" min="0" step="0.0001" name="balance" placeholder="Contoh 500000"><p class="help">Isi sesuai saldo pada bot supplier. Setelah order berhasil nilai ini otomatis berkurang sesuai modal × jumlah.</p></div><div class="field"><label class="label">Mata Uang Saldo</label><select class="select" name="currency"><option value="IDR">IDR / Rupiah</option><option value="USD">USD / Dollar</option><option value="USDT">USDT</option></select></div></div><input type="hidden" name="worker_profile" value="vercel-ondemand">
        <div class="field"><label class="label">Config Tambahan (JSON, opsional)</label><textarea class="textarea" name="flow_config" placeholder="{}"></textarea><p class="help">Saldo sengaja dibuat manual agar Vercel tidak perlu membuka menu saldo supplier setiap saat.</p></div>
        <div class="buttonRow"><button class="btn lime" type="submit">Simpan Supplier Telegram</button><button class="btn yellow" id="resetTelegramSupplierConnector" type="button">Form Baru</button></div>
      </form>
      <div id="telegramSupplierConnectorList" class="supplierGrid" style="margin-top:14px"></div>
    </div>
    <div class="panel">
      <h2 class="sectionTitle">Produk dari Bot Supplier Telegram</h2>
      <p class="help">Produk supplier dibuat manual karena bot Telegram tidak mempunyai endpoint katalog standar. Flow order dapat berbeda per produk dan mendukung klik tombol, kirim jumlah, menunggu pesan, dan mengambil hasil akun/key.</p>
      <form id="telegramSupplierProductForm" class="form">
        <input type="hidden" name="id">
        <div class="row3"><div class="field"><label class="label">Supplier</label><select class="select" name="connector_id" id="telegramSupplierProductConnector" required></select></div><div class="field"><label class="label">Nama Produk Supplier</label><input class="input" name="name" placeholder="Zoom 7 Hari" required></div><div class="field"><label class="label">Kode Supplier (opsional)</label><input class="input" name="external_code" placeholder="ZOOM7"></div></div>
        <div class="row3"><div class="field"><label class="label">Modal per Item</label><input class="input" type="number" min="0" step="0.0001" name="cost_amount" value="0"></div><div class="field"><label class="label">Currency</label><select class="select" name="currency"><option value="IDR">IDR</option><option value="USD">USD</option><option value="USDT">USDT</option></select></div><div class="field"><label class="label">Perhitungan Stok</label><select class="select" name="stock_mode"><option value="balance">Saldo + stok supplier bila diisi</option><option value="fixed">Stok supplier saja</option><option value="unlimited">Berdasarkan saldo / unlimited jika modal 0</option></select></div></div>
        <div class="field"><label class="label">Stok Aktual Supplier / Cache</label><input class="input" type="number" min="0" step="1" name="stock" placeholder="Akan diperbarui otomatis oleh Flow Cek Stok jika diisi"></div>
        <div class="field"><label class="label">Instruksi Alur untuk AI (opsional)</label><textarea class="textarea tall" name="ai_flow_instructions" placeholder="Contoh: /start, pilih tombol keyboard nomor 2 ALIGHT MOTION, jika qty > 1 klik 📝 lalu kirim qty, klik Buy(Saldo), Skip, Konfirmasi & proses, ambil semua teks setelah ACCOUNT DETAIL. Stok ada di tombol ALIGHT MOTION (74)."></textarea><div class="buttonRow" style="margin-top:8px"><button class="btn cyan" id="generateSupplierFlowAi" type="button">✨ Susun / Perbaiki Flow dengan AI</button></div><p class="help" id="supplierAiFlowNote">AI akan mengisi draft field Flow Cek Stok, Stock Regex, Flow Order, dan Regex Hasil. Periksa sebelum Simpan.</p></div>
        <div class="field"><label class="label">Flow Cek Stok (JSON, opsional)</label><textarea class="textarea tall" name="stock_flow" placeholder='[{"type":"start"},{"type":"capture","source":"buttons"}]'></textarea><p class="help">Flow ini dijalankan on-demand ketika user membuka produk/varian. Untuk stok yang tampil di tombol keyboard, gunakan capture dengan source "buttons". Kosongkan jika bot supplier tidak menampilkan stok.</p></div>
        <div class="row3"><div class="field"><label class="label">Stock Regex</label><input class="input" name="stock_regex" placeholder="Contoh: Stok[^0-9]*(\d+)"></div><div class="field"><label class="label">Cache Stok (detik)</label><input class="input" type="number" min="5" max="600" step="1" name="stock_cache_seconds" value="60"></div><div class="field"><label class="label">Tes Stok</label><button class="btn cyan" id="testTelegramSupplierStock" type="button">Cek Stok Sekarang</button></div></div>
        <div class="field"><label class="label">Flow Order (JSON)</label><textarea class="textarea tall" name="order_flow" required placeholder='[{"type":"start"},{"type":"click","button_index":2,"expect_text":"ALIGHT MOTION"},{"type":"click","text":"📝","when":{"quantity_gt":1}},{"type":"send","text":"{{quantity}}","when":{"quantity_gt":1}},{"type":"click","text":"Buy(Saldo)"},{"type":"click","text":"⏭️ Skip"},{"type":"click","text":"✅ Konfirmasi & proses","commit":true,"wait_after":false},{"type":"wait","contains":"〔 ACCOUNT DETAIL 〕","capture_delivery":true,"delivery_regex":"/〔 ACCOUNT DETAIL 〕\\s*([\\s\\S]*)/i","timeout_ms":45000}]'></textarea><p class="help"><code>button_index</code> dihitung mulai 1 dari tombol pertama. Kondisi jumlah dapat memakai <code>when:{"quantity_gt":1}</code>, <code>quantity_eq</code>, <code>quantity_gte</code>, <code>quantity_lt</code>, atau <code>quantity_lte</code>.</p></div>
        <div class="field"><label class="label">Regex Hasil Produk (opsional)</label><input class="input" name="delivery_regex" placeholder="Contoh: AKUN\s*:\s*([\s\S]+)"><p class="help">Kosong = seluruh teks pesan terakhir dianggap sebagai hasil produk.</p></div>
        <div class="field"><label class="label">Catatan Admin</label><textarea class="textarea" name="notes"></textarea></div>
        <div class="buttonRow"><button class="btn lime" type="submit">Simpan Produk Supplier</button><button class="btn yellow" id="resetTelegramSupplierProduct" type="button">Form Baru</button></div>
      </form>
      <div id="telegramSupplierProductList" class="supplierGrid" style="margin-top:14px"></div>
    </div>
    <div class="panel">
      <h2 class="sectionTitle">Order Supplier Terakhir</h2>
      <p class="help">Jika pelanggan sudah membayar tetapi supplier Telegram gagal sebelum tombol pembelian, perbaiki flow/stok lalu tekan <b>Retry Supplier</b>. Jika status <b>Manual Review</b>, cek bot supplier dulu agar tidak membeli dua kali.</p>
...
(function(){
  ...
  var state = { stats:{}, products:[], orders:[], poOrders:[], users:[], vouchers:[], polls:[], settings:{}, analytics:{}, maintenance:{}, backups:[], promos:[], deepStats:{}, license:{}, promoTargets:[], supplierStatus:{}, supplierProducts:[], supplierOrders:[], telegramSuppliers:{connectors:[],products:[]}, supplierLoaded:false, supplierAi:{configured:false} };
  ...
  function mergeVariantStockArray(product, mode){
    ...
    return existing.map(function(v,i){
      ...
      var isSupplierVariant=['prodseller','telegram_userbot'].includes(String(v.supplier_source||'').toLowerCase()) && String(v.supplier_product_id||'').trim();
      var stock=isSupplierVariant ? [] : (mode==='append' ? variantStock(v).concat(input) : input);
      return {
        name:v.name||v.nama||'',
  ...
  function renderSupplier(){
    ...
    if(list){
      if(!st.configured) list.innerHTML='<div class="empty">Atur PRODSELLER_API_KEY terlebih dahulu untuk memuat katalog supplier.</div>';
        ...
        var normalProducts=(state.products||[]).filter(function(x){return !['prodseller','telegram_userbot'].includes(String(x.supplier_source||'').toLowerCase());});
    ...
    if(orderList){ orderList.innerHTML=(state.supplierOrders||[]).map(function(o){
      ...
      var isTg=String(o.supplier||'').toLowerCase()==='telegram_userbot'; var supplierName=isTg?'Telegram Userbot':'ProdSeller'; var amount=isTg?(esc(o.amount_currency||'')+' '+esc(Number((o.raw_response||{}).cost_amount||0))):usdt(o.amount_usdt||0);
      return '<div class="supplierOrder '+cls+'"><b>'+esc(displayRef(o.order_ref||'-'))+'</b> · <span class="chip '+(cls==='delivered'?'green':(cls==='error'?'red':'yellow'))+'">'+esc(statusText)+'</span><br><b>'+supplierName+'</b> · Qty '+esc(o.quantity||1)+' · '+amount+(isTg&&o.worker_id?'<br>Worker: '+esc(o.worker_id):'')+(o.error_message?'<br><b>Error:</b> '+esc(o.error_message):'')+(String(o.status||'')!=='delivered'?'<br><button class="btn small cyan" type="button" data-supplier-retry="'+esc(o.order_ref||'')+'" data-supplier-type="'+(isTg?'telegram':'prodseller')+'">Retry Supplier</button>':'')+'</div>';
    }).join('')||'<div class="empty">Belum ada order supplier.</div>'; }
    document.querySelectorAll('[data-supplier-retry]').forEach(function(btn){btn.onclick=async function(){ btn.disabled=true; var old=btn.textContent; btn.textContent='Retry...'; try{await api(btn.dataset.supplierType==='telegram'?'telegram-supplier-retry':'prodseller-retry',{order_ref:btn.dataset.supplierRetry}); toast('Retry supplier berhasil.'); await load(); await loadSupplier(true);}catch(e){toast(e.message,true);}finally{btn.disabled=false;btn.textContent=old;} };});
  }


  function prettyJson(value,fallback){ try{return JSON.stringify(value==null?fallback:value,null,2);}catch(e){return JSON.stringify(fallback||{},null,2);} }
  function renderTelegramSuppliers(){
    var data=state.telegramSuppliers||{connectors:[],products:[]}; var connectors=data.connectors||[]; var products=data.products||[];
    var connectorSelect=document.getElementById('telegramSupplierProductConnector');
    if(connectorSelect){ connectorSelect.innerHTML='<option value="">Pilih supplier...</option>'+connectors.map(function(c){return '<option value="'+esc(c.id)+'">'+esc(c.name)+' · '+esc(c.bot_username)+'</option>';}).join(''); }
    var connectorList=document.getElementById('telegramSupplierConnectorList');
    if(connectorList){ connectorList.innerHTML=connectors.map(function(c){ var bal=c.balance==null?'belum diisi':(String(c.currency||'IDR')+' '+Number(c.balance||0).toLocaleString('id-ID')); return '<article class="supplierCard"><div class="supplierCardTop"><div class="supplierThumbFallback">🤖</div><div><h3>'+esc(c.name)+'</h3><div class="supplierMeta">'+esc(c.bot_username)+' · <span class="chip '+(c.enabled?'green':'red')+'">'+(c.enabled?'AKTIF':'NONAKTIF')+'</span></div></div></div><div class="detailGrid"><div class="detailItem"><b>Saldo</b><br>'+esc(bal)+'</div><div class="detailItem"><b>Mode</b><br>Vercel On-Demand</div><div class="detailItem"><b>Status</b><br>'+esc(c.status||'offline')+'</div></div>'+(c.last_error?'<p class="help" style="color:#991b1b"><b>Error:</b> '+esc(c.last_error)+'</p>':'')+'<div class="buttonRow"><button class="btn small cyan" type="button" data-tg-connector-edit="'+esc(c.id)+'">Edit</button><button class="btn small red" type="button" data-tg-connector-delete="'+esc(c.id)+'">Hapus</button></div></article>'; }).join('')||'<div class="empty">Belum ada Telegram supplier. Jalankan SQL v81 lalu SQL v82, kemudian tambahkan supplier pertama.</div>'; }
    var normalProducts=(state.products||[]).filter(function(x){return !['prodseller','telegram_userbot'].includes(String(x.supplier_source||'').toLowerCase());});
    var productList=document.getElementById('telegramSupplierProductList');
    if(productList){ productList.innerHTML=products.map(function(p){ var mode=p.link_type==='variant'?'variant':'product'; var targetOptions='<option value="">Pilih produk induk...</option>'+normalProducts.map(function(x){return '<option value="'+esc(x.kode)+'" '+(mode==='variant'&&String(p.local_code||'')===String(x.kode||'')?'selected':'')+'>'+esc(x.nama)+' · '+esc(x.kode)+'</option>';}).join(''); var sell=Math.max(1000,Number(p.local_price||0)||1000); return '<article class="supplierCard" data-tg-product-card="'+esc(p.id)+'"><h3>'+esc(p.name)+'</h3><div class="supplierMeta">'+esc(p.connector_name||'-')+' · '+esc(p.bot_username||'')+'<br>Modal '+esc(p.currency||'IDR')+' '+esc(Number(p.cost_amount||0))+' · Stok '+(p.stock==null?'dari saldo':esc(p.stock))+' '+(p.selected?'<span class="chip green">TERHUBUNG</span>':'')+'</div><div class="field"><label class="label">Harga Jual iLink</label><input class="input" type="number" min="1000" step="500" data-tg-selling-price value="'+esc(sell)+'"></div><div class="field"><label class="label">Masukkan Sebagai</label><select class="select" data-tg-target-mode><option value="product" '+(mode==='product'?'selected':'')+'>Produk mandiri</option><option value="variant" '+(mode==='variant'?'selected':'')+'>Varian produk yang sudah ada</option></select></div><div class="field" data-tg-target-wrap '+(mode==='variant'?'':'style="display:none"')+'><label class="label">Produk Induk</label><select class="select" data-tg-target-product>'+targetOptions+'</select><label class="label" style="margin-top:8px">Nama Varian</label><input class="input" data-tg-variant-name value="'+esc(p.local_variant_name||p.name||'')+'"></div><div class="buttonRow"><button class="btn small lime" type="button" data-tg-product-link="'+esc(p.id)+'">'+(p.selected?'Update Link':'Resellerkan')+'</button><button class="btn small cyan" type="button" data-tg-product-edit="'+esc(p.id)+'">Edit Flow</button><button class="btn small red" type="button" data-tg-product-delete="'+esc(p.id)+'">Hapus</button></div></article>'; }).join('')||'<div class="empty">Belum ada produk Telegram supplier.</div>'; }

    document.querySelectorAll('[data-tg-target-mode]').forEach(function(sel){sel.onchange=function(){var card=sel.closest('[data-tg-product-card]');var wrap=card&&card.querySelector('[data-tg-target-wrap]');if(wrap)wrap.style.display=sel.value==='variant'?'block':'none';};});
    document.querySelectorAll('[data-tg-connector-edit]').forEach(function(btn){btn.onclick=function(){var c=connectors.find(function(x){return String(x.id)===String(btn.dataset.tgConnectorEdit);});var f=document.getElementById('telegramSupplierConnectorForm');if(!c||!f)return;['id','code','name','bot_username','worker_profile','currency','balance'].forEach(function(k){if(f[k])f[k].value=c[k]==null?'':c[k];});f.enabled.value=c.enabled?'true':'false';f.flow_config.value=prettyJson(c.flow_config||{},{});f.scrollIntoView({behavior:'smooth',block:'center'});};});
    document.querySelectorAll('[data-tg-connector-delete]').forEach(function(btn){btn.onclick=async function(){if(!confirm('Hapus supplier Telegram ini beserta produk mapping-nya?'))return;try{await api('telegram-supplier-delete',{id:btn.dataset.tgConnectorDelete});toast('Supplier Telegram dihapus.');state.supplierLoaded=false;await loadSupplier(true);}catch(e){toast(e.message,true);}};});
    document.querySelectorAll('[data-tg-product-edit]').forEach(function(btn){btn.onclick=function(){var row=products.find(function(x){return String(x.id)===String(btn.dataset.tgProductEdit);});var f=document.getElementById('telegramSupplierProductForm');if(!row||!f)return;['id','connector_id','name','external_code','cost_amount','currency','stock_mode','stock','delivery_regex','stock_regex','stock_cache_seconds','notes'].forEach(function(k){if(f[k])f[k].value=row[k]==null?'':row[k];});f.order_flow.value=prettyJson(row.order_flow||[],[]);if(f.stock_flow)f.stock_flow.value=prettyJson(row.stock_flow||[],[]);f.scrollIntoView({behavior:'smooth',block:'center'});};});
    document.querySelectorAll('[data-tg-product-delete]').forEach(function(btn){btn.onclick=async function(){if(!confirm('Hapus produk supplier ini? Produk iLink yang sudah terhubung sebaiknya diedit dulu jika masih dijual.'))return;try{await api('telegram-supplier-product-delete',{id:btn.dataset.tgProductDelete});toast('Produk supplier dihapus.');state.supplierLoaded=false;await loadSupplier(true);}catch(e){toast(e.message,true);}};});
    document.querySelectorAll('[data-tg-product-link]').forEach(function(btn){btn.onclick=async function(){var card=btn.closest('[data-tg-product-card]');var price=Number(card.querySelector('[data-tg-selling-price]').value||0);var mode=card.querySelector('[data-tg-target-mode]').value;var target=card.querySelector('[data-tg-target-product]');var variant=card.querySelector('[data-tg-variant-name]');if(price<1000)return toast('Harga jual minimal Rp1.000.',true);if(mode==='variant'&&!target.value)return toast('Pilih produk induk.',true);try{await api('telegram-supplier-import',{supplier_product_ref:btn.dataset.tgProductLink,selling_price:price,target_mode:mode,target_product_code:target.value||'',variant_name:variant.value||''});toast('Produk Telegram supplier berhasil dihubungkan.');await load();state.supplierLoaded=false;await loadSupplier(true);}catch(e){toast(e.message,true);}};});
  }
  ...
  function renderSupplierAi(){
    ...
    if(f){if(f.base_url)f.base_url.value='https://generativelanguage.googleapis.com/v1beta/openai';if(f.model)f.model.value=(/^gemini-|^gemma-/i.test(String(c.model||''))?c.model:'gemini-3.7-flash');if(f.backend)f.backend.value='chat_completions';if(f.api_key)f.api_key.value='';}
  ...
  async function loadSupplier(force){
    if(state.supplierLoaded && !force){ renderSupplier(); renderTelegramSuppliers(); return; }
    ...
    state.telegramSuppliers=await apiSafe('telegram-suppliers',{connectors:[],products:[]});
    state.supplierAi=await apiSafe('supplier-ai-config',{configured:false,base_url:'https://generativelanguage.googleapis.com/v1beta/openai',model:'gemini-3.7-flash',backend:'chat_completions'});
    renderSupplierAi();
    state.supplierLoaded=true;
    renderSupplier();
    renderTelegramSuppliers();
  }
  ...
  function renderProducts(){
    ...
    document.getElementById('productList').innerHTML=rows.map(function(p){
      var supplierType=String(p.supplier_source||'').toLowerCase();
      var isSupplier=['prodseller','telegram_userbot'].includes(supplierType);
      var supplierBadge=supplierType==='prodseller'?'SUPPLIER OTOMATIS · PRODSELLER':(supplierType==='telegram_userbot'?'SUPPLIER OTOMATIS · TELEGRAM':'SUPPLIER OTOMATIS');
      var isPo=!isSupplier && String(p.delivery_mode||'auto')==='po';
      var varsArr=productVariants(p);
      var hasSupplierVariants=varsArr.some(function(v){return ['prodseller','telegram_userbot'].includes(String(v.supplier_source||'').toLowerCase()) && String(v.supplier_product_id||'').trim();});
      var visibleVars=varsArr.slice(0,3);
      var vars=visibleVars.map(function(v){
        var supplierVariant=['prodseller','telegram_userbot'].includes(String(v.supplier_source||'').toLowerCase()) && String(v.supplier_product_id||'').trim();
        var mode=supplierVariant?('SUPPLIER · stok '+Math.max(0,Number(v.supplier_stock||0))):(String(v.delivery_mode||p.delivery_mode||'auto')==='po'?'PO':(variantStock(v).length+' stok'));
      ...
      var availability=isSupplier?'SUPPLIER OTOMATIS':(hasSupplierVariants?(isPo?'PRE-ORDER + SUPPLIER':'AUTO + SUPPLIER'):(isPo?'PRE-ORDER':('STOK '+stockCount(p))));
      var hasLocalStock=!isSupplier && (!varsArr.length ? !isPo : varsArr.some(function(v){return !(['prodseller','telegram_userbot'].includes(String(v.supplier_source||'').toLowerCase()) && String(v.supplier_product_id||'').trim()) && String(v.delivery_mode||p.delivery_mode||'auto')!=='po';}));
      ...
      return '<article class="product '+(isPo?'poProduct ':'')+(p.active===false?'productOff':'')+'">'+
        '<div class="productTop">'+productMediaHtml(p)+'<div class="productInfo"><h3>'+esc(p.nama)+'</h3><div class="subtle">'+esc(p.category||'Produk')+' - '+availability+(varsArr.length?' - '+varsArr.length+' varian':'')+'<br><span class="scopeBadge '+(p.display_scope==='marketplace'?'market':'')+'">'+(p.display_scope==='marketplace'?'MARKETPLACE SAJA':'BOT + MARKETPLACE')+'</span>'+(isSupplier?'<br><span class="deliveryModeBadge">'+esc(supplierBadge)+'</span>':(hasSupplierVariants?'<br><span class="deliveryModeBadge">MEMILIKI VARIAN SUPPLIER OTOMATIS</span>':(isPo?'<br><span class="deliveryModeBadge">PRE-ORDER · KIRIM MANUAL</span>':'')))+'</div></div><button class="statusToggle '+(p.active===false?'off':'')+'" data-toggle-product="'+esc(p.kode)+'">'+(p.active===false?'OFF':'ON')+'</button></div>'+ 
  ...
  function editVariantCardHtml(v,i, allowRemove){
    v=v||{};
    var sku=String(v.sku||v.kode||('VAR'+(i+1))).toUpperCase();
    var supplierVariantType=String(v.supplier_source||'').toLowerCase();
    var supplierVariant=['prodseller','telegram_userbot'].includes(supplierVariantType) && String(v.supplier_product_id||'').trim();
    var supplierVariantText=supplierVariantType==='prodseller'?'SUPPLIER OTOMATIS · PRODSELLER · stok mengikuti saldo + stok ProdSeller.':'SUPPLIER OTOMATIS · TELEGRAM · stok mengikuti saldo + stok supplier Telegram.';
    var deliveryField=supplierVariant
      ? '<div class="field"><label class="label">Sistem Pengiriman Varian</label><div class="variantMainCompact">'+esc(supplierVariantText)+'</div><p class="help">Link supplier tetap dipertahankan saat nama, harga, deskripsi, atau SnK varian diedit.</p></div>'
  ...
  function collectEditVariants(product){
    ...
    document.querySelectorAll('[data-edit-variant-card]').forEach(function(card,i){
      ...
      var name=val('name');
      if(name){ var isSupplierVariant=['prodseller','telegram_userbot'].includes(String(old.supplier_source||'').toLowerCase()) && String(old.supplier_product_id||'').trim(); rows.push({
        name:name,
        price:val('price'),
        cost_price:val('cost'),
        sku:sku,
        stock:isSupplierVariant?[]:variantStock(old),
        bulk_prices:parseBulkArray(val('bulk')),
        description:val('description'),
        snk:val('snk'),
  ...
  function editFormHtml(p){
    ...
    var editSupplierType=String(p.supplier_source||'').toLowerCase();
    var isSupplier=['prodseller','telegram_userbot'].includes(editSupplierType);
    var variantCards=(p.variants||[]).map(function(v,i){return editVariantCardHtml(v,i,true);}).join('');
    var editSupplierLabel=editSupplierType==='prodseller'?'SUPPLIER OTOMATIS · PRODSELLER':(editSupplierType==='telegram_userbot'?'SUPPLIER OTOMATIS · TELEGRAM':'SUPPLIER OTOMATIS');
    var editSupplierStockText=editSupplierType==='prodseller'?'OTOMATIS · stok mengikuti saldo + stok ProdSeller':'OTOMATIS · stok mengikuti saldo + stok supplier Telegram';
    var deliveryEditor=isSupplier
      ? '<input type="hidden" name="delivery_mode" value="po"><div class="row"><div class="field"><label class="label">Sistem Pengiriman</label><div class="variantMainCompact">'+esc(editSupplierLabel)+' · produk langsung diambil dari supplier setelah pembayaran, bukan Pesanan PO manual.</div></div><div class="field"><label class="label">Mode Aktif</label><div class="variantMainCompact">'+esc(editSupplierStockText)+'</div></div></div>'
  ...
  function openStockProduct(code){
    ...
    if(hasVar){
      html += '<div class="variantList">'+(p.variants||[]).map(function(v,i){ var sku=String(v.sku||v.kode||('VAR'+(i+1))).toUpperCase(); var supplierVariant=['prodseller','telegram_userbot'].includes(String(v.supplier_source||'').toLowerCase())&&String(v.supplier_product_id||'').trim(); if(supplierVariant) return '<div class="variantCard"><h3>'+esc(v.name||v.nama||sku)+' <span class="chip green">SUPPLIER</span></h3><p class="help">Stok varian ini mengikuti saldo + stok supplier dan tidak diisi manual.</p></div>'; return '<div class="variantCard"><h3>'+esc(v.name||v.nama||sku)+' <span class="chip '+(variantActive(v)?'green':'red')+'">'+(variantActive(v)?'ON':'OFF')+'</span></h3><p class="help">Stok sekarang: '+variantStock(v).length+'</p><label class="label">Tambah Stok Varian</label><textarea class="textarea" data-stock-field="'+esc(sku)+'" placeholder="Satu stok per baris atau pisahkan koma\nakun1:pass1\nakun2:pass2"></textarea></div>'; }).join('')+'</div>';
    } else {
  ...
  function openManageProduct(code){
    ...
    if(hasVar){
      html += '<div class="variantList">'+(p.variants||[]).map(function(v,i){ var sku=String(v.sku||v.kode||('VAR'+(i+1))).toUpperCase(); var supplierVariant=['prodseller','telegram_userbot'].includes(String(v.supplier_source||'').toLowerCase())&&String(v.supplier_product_id||'').trim(); if(supplierVariant) return '<div class="variantCard"><h3>'+esc(v.name||v.nama||sku)+' <span class="chip green">SUPPLIER</span></h3><p class="help">Harga: '+rupiah(v.price||v.harga||p.harga)+' · stok otomatis dari supplier. Tidak dapat diubah dari Kelola Stok.</p></div>'; return '<div class="variantCard"><h3>'+esc(v.name||v.nama||sku)+' <span class="chip '+(variantActive(v)?'green':'red')+'">'+(variantActive(v)?'ON':'OFF')+'</span></h3><p class="help">Harga: '+rupiah(v.price||v.harga||p.harga)+' | Grosir: '+esc(variantBulkText(v)||'-')+'</p><label class="label">Stok Varian</label><textarea class="textarea tall" data-stock-field="'+esc(sku)+'" placeholder="Satu stok per baris">'+esc(variantStock(v).join('\n'))+'</textarea></div>'; }).join('')+'</div>';
    } else {
  ...
  function applyGeminiModels(data){
    var d=data||{};var models=Array.isArray(d.models)?d.models:[];var selected=String(d.selected||'').trim();
    var list=document.getElementById('geminiModelList');var f=document.getElementById('supplierAiConfigForm');var hint=document.getElementById('supplierAiModelHint');
    if(list&&models.length)list.innerHTML=models.map(function(m){return '<option value="'+esc(m)+'">';}).join('');
    if(f&&f.model&&selected)f.model.value=selected;
    if(hint)hint.textContent=models.length?('Model tersedia untuk API key ini: '+models.slice(0,8).join(', ')+(models.length>8?' ...':'')):('Belum ada daftar model dari Google.');
  }
  var loadSupplierAiModels=document.getElementById('loadSupplierAiModels'); if(loadSupplierAiModels) loadSupplierAiModels.onclick=async function(){loadSupplierAiModels.disabled=true;var old=loadSupplierAiModels.textContent;loadSupplierAiModels.textContent='Memuat...';try{var r=await api('supplier-ai-models');applyGeminiModels(r.data||{});toast('Model Google berhasil dimuat. Dipilih: '+String((r.data&&r.data.selected)||'-'));}catch(err){toast(err.message,true);}finally{loadSupplierAiModels.disabled=false;loadSupplierAiModels.textContent=old;}};
  var supplierAiConfigForm=document.getElementById('supplierAiConfigForm'); if(supplierAiConfigForm) supplierAiConfigForm.onsubmit=async function(e){e.preventDefault();var d=formDataRaw(e.target);try{var r=await api('supplier-ai-config-save',d);state.supplierAi=r.data||{};renderSupplierAi();toast('Pengaturan Google Gemini disimpan.');}catch(err){toast(err.message,true);}};
  var testSupplierAi=document.getElementById('testSupplierAi'); if(testSupplierAi) testSupplierAi.onclick=async function(){testSupplierAi.disabled=true;var old=testSupplierAi.textContent;testSupplierAi.textContent='Mencari model & mengetes...';try{var r=await api('supplier-ai-test',{});applyGeminiModels(r.data||{});toast((r.data&&r.data.ok?'Gemini terhubung':'Gemini merespons')+' · Model: '+String((r.data&&r.data.model)||'-'));state.supplierAi=await apiSafe('supplier-ai-config',state.supplierAi||{});renderSupplierAi();if(r.data)applyGeminiModels({models:r.data.models||[],selected:r.data.model});}catch(err){toast(err.message,true);}finally{testSupplierAi.disabled=false;testSupplierAi.textContent=old;}};
  var generateSupplierFlowAi=document.getElementById('generateSupplierFlowAi'); if(generateSupplierFlowAi) generateSupplierFlowAi.onclick=async function(){var f=document.getElementById('telegramSupplierProductForm');if(!f)return;var instructions=String((f.ai_flow_instructions&&f.ai_flow_instructions.value)||'').trim();if(!instructions)return toast('Tuliskan langkah bot supplier pada Instruksi Alur untuk AI.',true);var parse=function(v,fb){try{return JSON.parse(v||'');}catch(e){return fb;}};generateSupplierFlowAi.disabled=true;var old=generateSupplierFlowAi.textContent;generateSupplierFlowAi.textContent='AI sedang menyusun...';try{var r=await api('supplier-ai-generate-flow',{instructions:instructions,product_name:f.name.value||'',external_code:f.external_code.value||'',stock_flow:parse(f.stock_flow.value,[]),stock_regex:f.stock_regex.value||'',order_flow:parse(f.order_flow.value,[]),delivery_regex:f.delivery_regex.value||''});var d=r.data||{};f.stock_flow.value=prettyJson(d.stock_flow||[],[]);f.stock_regex.value=d.stock_regex||'';f.order_flow.value=prettyJson(d.order_flow||[],[]);f.delivery_regex.value=d.delivery_regex||'';var note=document.getElementById('supplierAiFlowNote');if(note)note.textContent=(d.notes||'Flow AI dibuat. Periksa sebelum Simpan.')+(d.model?' · Model: '+d.model:'');toast('Draft flow dari AI sudah dimasukkan ke form. Periksa lalu Simpan.');}catch(err){toast(err.message,true);}finally{generateSupplierFlowAi.disabled=false;generateSupplierFlowAi.textContent=old;}};

  var telegramSupplierConnectorForm=document.getElementById('telegramSupplierConnectorForm'); if(telegramSupplierConnectorForm) telegramSupplierConnectorForm.onsubmit=async function(e){e.preventDefault();var d=formDataRaw(e.target);try{d.flow_config=d.flow_config?JSON.parse(d.flow_config):{};}catch(_){return toast('Flow config supplier bukan JSON valid.',true);}try{await api('telegram-supplier-save',d);toast('Supplier Telegram disimpan.');e.target.reset();e.target.worker_profile.value='vercel-ondemand';state.supplierLoaded=false;await loadSupplier(true);}catch(err){toast(err.message,true);}};
  var resetTelegramSupplierConnector=document.getElementById('resetTelegramSupplierConnector'); if(resetTelegramSupplierConnector) resetTelegramSupplierConnector.onclick=function(){var f=document.getElementById('telegramSupplierConnectorForm');if(f){f.reset();f.id.value='';f.worker_profile.value='vercel-ondemand';}};
  var telegramSupplierProductForm=document.getElementById('telegramSupplierProductForm'); if(telegramSupplierProductForm) telegramSupplierProductForm.onsubmit=async function(e){e.preventDefault();var d=formDataRaw(e.target);try{d.order_flow=d.order_flow?JSON.parse(d.order_flow):[];d.stock_flow=d.stock_flow?JSON.parse(d.stock_flow):[];}catch(_){return toast('Flow order / flow stok bukan JSON valid.',true);}try{await api('telegram-supplier-product-save',d);toast('Produk Telegram supplier disimpan.');e.target.reset();e.target.id.value='';state.supplierLoaded=false;await loadSupplier(true);}catch(err){toast(err.message,true);}};
  var testTelegramSupplierStock=document.getElementById('testTelegramSupplierStock'); if(testTelegramSupplierStock) testTelegramSupplierStock.onclick=async function(){var f=document.getElementById('telegramSupplierProductForm');var id=f&&f.id?String(f.id.value||'').trim():'';if(!id)return toast('Simpan produk supplier dulu sebelum tes stok.',true);testTelegramSupplierStock.disabled=true;var old=testTelegramSupplierStock.textContent;testTelegramSupplierStock.textContent='Mengecek...';try{var r=await api('telegram-supplier-stock-check',{id:id});var d=(r&&r.data)||{};var supplier=d.supplierStock==null?(d.product&&d.product.stock):d.supplierStock;var effective=d.availableStock==null?d.stock:d.availableStock;toast('Stok supplier terdeteksi: '+String(supplier==null?'-':supplier)+' · stok efektif: '+String(effective==null?'-':effective));state.supplierLoaded=false;await loadSupplier(true);}catch(e){toast(e.message,true);}finally{testTelegramSupplierStock.disabled=false;testTelegramSupplierStock.textContent=old;}};
  var resetTelegramSupplierProduct=document.getElementById('resetTelegramSupplierProduct'); if(resetTelegramSupplierProduct) resetTelegramSupplierProduct.onclick=function(){var f=document.getElementById('telegramSupplierProductForm');if(f){f.reset();f.id.value='';}};
