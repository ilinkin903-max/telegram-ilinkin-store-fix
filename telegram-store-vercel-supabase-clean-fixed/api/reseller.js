  ...
  <section id="workflowSettings" class="section">
    ...
    <div class="panel" id="workflowRecorderPanel">
      ...
      <div class="workflowRecorder">
        <div>
          <div class="workflowMessage"><b>Pesan bot supplier yang diterima</b><p class="help">Jika supplier mengirim 2 pesan atau lebih, pilih <b>pesan yang benar-benar dipakai untuk step ini</b>. Tombol pada setiap pesan tetap dapat dipilih langsung.</p><div id="workflowLastMessage"><div class="empty">Belum ada balasan. Pilih kategori teks lalu kirim /start atau perintah lain.</div></div></div>
          ...
          <div class="actions" style="margin-top:9px"><button class="btn cyan" id="workflowSendText" type="button">✍️ Kirim Teks & Rekam</button><button class="btn yellow" id="workflowRefreshMessage" type="button">🔄 Refresh Pesan Supplier</button><button class="btn lime" id="workflowMarkResult" type="button" title="Balasan Ini = Hasil Produk">📦 Pesan Terpilih = Hasil Produk</button></div>
        </div>
...
(function(){
  ...
  function openWorkflowStepEdit(stepId){
    var w=(state.workflowDetail||{}).workflow;var step=((state.workflowDetail||{}).steps||[]).find(function(x){return String(x.id)===String(stepId||'');});if(!w||!step)return toast('Step tidak ditemukan.',true);
    var type=String(step.action_type||'text');var cat=String(step.text_category||'other');var body='<form id="workflowStepEditForm" class="form"><input type="hidden" name="workflow_id" value="'+esc(w.id)+'"><input type="hidden" name="step_id" value="'+esc(step.id)+'"><div class="field"><label class="label">Jenis Step</label><select class="select" name="action_type" id="stepEditActionType"><option value="button" '+(type==='button'?'selected':'')+'>🔘 Klik Tombol</option><option value="text" '+(type!=='button'?'selected':'')+'>✍️ Kirim Teks</option></select></div><div class="field" id="stepEditTextCategoryBox"><label class="label">Kategori Teks</label><select class="select" name="text_category" id="stepEditTextCategory"><option value="other" '+(cat!=='quantity'?'selected':'')+'>Teks / Perintah Lainnya</option><option value="quantity" '+(cat==='quantity'?'selected':'')+'>Jumlah Pembelian</option></select></div><div class="field" id="stepEditValueBox"><label class="label" id="stepEditValueLabel">'+(type==='button'?'Teks Tombol':'Teks yang Dikirim')+'</label><input class="input" name="action_value" id="stepEditValue" value="'+esc(step.action_value||'')+'" placeholder="'+(type==='button'?'Contoh: ✅ Konfirmasi':'Contoh: /start atau Tidak')+'"><p class="help" id="stepEditHelp">'+(cat==='quantity'?'Kategori Jumlah Pembelian selalu memakai {quantity} saat order asli.':'Teks/tombol ini yang dicari atau dikirim saat workflow berjalan.')+'</p></div>'+(w.active?'<div class="workflowSelectionWarning">Mengedit step pada workflow aktif akan menonaktifkan workflow agar order baru tidak membaca langkah yang sedang diubah.</div>':'')+'<button class="btn cyan" type="submit">Simpan Step</button></form>';
    openModal('Edit Step '+step.step_order,body);var typeEl=document.getElementById('stepEditActionType'),catEl=document.getElementById('stepEditTextCategory'),box=document.getElementById('stepEditTextCategoryBox'),value=document.getElementById('stepEditValue'),label=document.getElementById('stepEditValueLabel'),help=document.getElementById('stepEditHelp');function sync(){var t=String(typeEl.value||'text'),c=String(catEl.value||'other');box.classList.toggle('hidden',t==='button');label.textContent=t==='button'?'Teks Tombol':'Teks yang Dikirim';if(t==='text'&&c==='quantity'){value.value='{quantity}';value.readOnly=true;help.textContent='Jumlah diambil otomatis dari quantity order pembeli.';}else{value.readOnly=false;if(value.value==='{quantity}')value.value='';help.textContent=t==='button'?'Masukkan teks tombol supplier persis seperti yang akan diklik.':'Masukkan teks/perintah yang akan dikirim ke supplier.';}}typeEl.onchange=sync;catEl.onchange=sync;sync();document.getElementById('workflowStepEditForm').onsubmit=async function(e){e.preventDefault();var d=formDataRaw(e.target);try{await api('workflow-step-update',d);closeModal();await reloadWorkflowAndSuppliers(w.id);toast('Step diperbarui. Periksa urutan dan aktifkan workflow kembali bila sudah benar.');}catch(err){toast(err.message,true);}};
  }
  ...
  function renderWorkflow(){
    ...
    var title=document.getElementById('workflowRecorderTitle'); if(title){var cs=current&&supplierById(current.supplier_id);var cstock=(cs&&Number(current.unit_cost_idr||0)>0)?Math.floor(Number(cs.manual_balance_idr||0)/Number(current.unit_cost_idr||1)):0;title.textContent=current?(current.name+' · '+workflowTargetLabel(current.product_code,current.variant_key)+' → '+((cs&&cs.name)||current.target_username)+' · Modal '+rupiah(current.unit_cost_idr||0)+' · Stok '+cstock+(current.active?' · AKTIF':' · MODE REKAM')):'Pilih atau buat workflow untuk mulai merekam.';}
    var snap=current?(current.last_message_snapshot||{}):{}; var candidates=current&&Array.isArray(current.recent_message_snapshots)?current.recent_message_snapshots:[]; if(!candidates.length&&snap&&snap.id)candidates=[snap]; var selectedMessageId=Number(current&&current.last_message_id||0); var msg=document.getElementById('workflowLastMessage');
    if(msg){
      var warning=(candidates.length>1&&!selectedMessageId)?'<div class="workflowSelectionWarning">⚠️ Supplier mengirim '+esc(candidates.length)+' pesan. Pilih satu pesan yang akan direkam sebelum melanjutkan step.</div>':'';
      msg.innerHTML=warning+(candidates.length?candidates.map(function(m,index){var selected=Number(m.id||0)===selectedMessageId; var btns=(m.buttons||[]).map(function(b){if(b.kind==='url')return '<button class="btn small" type="button" disabled>🔗 '+esc(b.text)+'</button>'; return '<button class="btn small purple" type="button" data-workflow-button="'+encodeURIComponent(String(b.text||''))+'" data-workflow-message-id="'+esc(m.id)+'">🔘 '+esc(b.text)+'</button>';}).join(''); return '<div class="workflowMessageChoice '+(selected?'selected':'pending')+'"><div class="messageHead"><b>Pesan '+esc(index+1)+' · ID '+esc(m.id)+'</b>'+(selected?'<span class="chip green">DIPILIH / DIREKAM</span>':'<button class="btn small yellow" type="button" data-workflow-select-message="'+esc(m.id)+'">Pilih Pesan Ini</button>')+'</div><pre>'+esc(m.text||'[Pesan tanpa teks]')+'</pre>'+(btns?'<div class="workflowButtons">'+btns+'</div>':'')+'</div>';}).join(''):'<div class="empty">Belum ada balasan. Pilih kategori teks lalu kirim /start atau perintah lain.</div>');
    }
    document.querySelectorAll('[data-workflow-select-message]').forEach(function(btn){btn.onclick=async function(){var id=workflowCurrentId(); if(!id)return toast('Pilih workflow dulu.',true); btn.disabled=true; try{await api('workflow-select-message',{workflow_id:id,message_id:Number(btn.dataset.workflowSelectMessage||0)}); state.workflowLoaded=false; await loadWorkflow(true); toast('Pesan dipilih sebagai balasan resmi step.');}catch(e){toast(e.message,true);}finally{btn.disabled=false;}};});
    document.querySelectorAll('[data-workflow-button]').forEach(function(btn){btn.onclick=async function(){var id=workflowCurrentId(); if(!id)return toast('Pilih workflow dulu',true); if(current&&current.active)return toast('Workflow aktif tidak dapat direkam ulang. Buat workflow baru untuk revisi.',true); btn.disabled=true; var old=btn.textContent; btn.textContent='Memproses...'; try{await api('workflow-action',{workflow_id:id,action_type:'button',action_value:decodeURIComponent(btn.dataset.workflowButton||''),message_id:Number(btn.dataset.workflowMessageId||0)}); state.workflowLoaded=false; await loadWorkflow(true);}catch(e){toast(e.message,true);}finally{btn.disabled=false;btn.textContent=old;}};});
    var steps=(state.workflowDetail&&state.workflowDetail.steps)||[]; var stepList=document.getElementById('workflowStepList');
    if(stepList) stepList.innerHTML=steps.map(function(step){var response=step.response_snapshot||{}; var responses=Array.isArray(step.response_snapshots)?step.response_snapshots:[]; var textLabel=step.action_type==='button'?'🔘 KLIK TOMBOL':(String(step.text_category||'')==='quantity'?'🔢 JUMLAH PEMBELIAN':'✍️ TEKS / LAINNYA'); var selectedIndex=response.id?responses.findIndex(function(x){return Number(x&&x.id||0)===Number(response.id||0);}):-1; return '<div class="workflowStep '+(step.capture_result?'result':'')+'"><div class="messageHead"><b>STEP '+esc(step.step_order)+' · '+textLabel+'</b><div class="actions"><button class="btn small cyan" type="button" data-workflow-step-edit="'+esc(step.id)+'">Edit</button><button class="btn small red" type="button" data-workflow-step-delete="'+esc(step.id)+'">Hapus</button></div></div>'+(step.capture_result?' <span class="chip green">HASIL PRODUK</span>':'')+'<br><code>'+esc(step.action_value)+'</code>'+(step.preview_value&&step.preview_value!==step.action_value?'<br><span class="subtle">Saat rekam dikirim: '+esc(step.preview_value)+'</span>':'')+(responses.length>1?'<br><span class="subtle">Supplier mengirim '+esc(responses.length)+' pesan · '+(selectedIndex>=0?'dipilih Pesan '+esc(selectedIndex+1):'<b>BELUM DIPILIH</b>')+'</span>':'')+(response.id?'<div class="stepResponse">Balasan direkam: '+esc(response.text||'[tanpa teks]')+'</div>':'<div class="stepResponse">'+(responses.length>1?'Pilih salah satu pesan supplier di panel kiri.':'Balasan baru belum terdeteksi.')+'</div>')+'</div>';}).join('')||'<div class="empty">Belum ada step.</div>';
    ...
    var list=document.getElementById('workflowList'); if(list) list.innerHTML=(state.workflows||[]).map(function(w){var supplier=supplierById(w.supplier_id);var stock=(supplier&&Number(w.unit_cost_idr||0)>0)?Math.floor(Number(supplier.manual_balance_idr||0)/Number(w.unit_cost_idr||1)):0; return '<div class="workflowCard '+(w.active?'active':'')+'"><b>'+esc(w.name||w.product_code)+'</b> '+(w.active?'<span class="chip green">AKTIF</span>':'<span class="chip yellow">DRAFT</span>')+'<br><span class="subtle">'+esc(workflowTargetLabel(w.product_code,w.variant_key))+' → '+esc((supplier&&supplier.name)||w.target_username)+'</span><br><span class="subtle">Modal '+rupiah(w.unit_cost_idr||0)+' · Stok perkiraan '+esc(stock)+'</span><div class="actions" style="margin-top:8px"><button class="btn small cyan" type="button" data-workflow-open="'+esc(w.id)+'">Buka</button><button class="btn small cyan" type="button" data-workflow-edit-card="'+esc(w.id)+'">Edit</button><button class="btn small purple" type="button" data-workflow-copy-card="'+esc(w.id)+'">Salin</button>'+(w.active?'<button class="btn small yellow" type="button" data-workflow-deactivate="'+esc(w.id)+'">Nonaktifkan</button>':'')+'</div></div>';}).join('')||'<div class="empty">Belum ada workflow.</div>';
  ...
  var workflowMarkResult=document.getElementById('workflowMarkResult'); if(workflowMarkResult) workflowMarkResult.onclick=async function(){var id=workflowCurrentId(); var currentWorkflow=state.workflowDetail&&state.workflowDetail.workflow; if(!id)return toast('Pilih workflow dulu.',true); if(!Number(currentWorkflow&&currentWorkflow.last_message_id||0))return toast('Pilih pesan supplier yang berisi produk terlebih dahulu.',true); try{await api('workflow-mark-result',{workflow_id:id,message_id:Number(currentWorkflow.last_message_id)}); state.workflowLoaded=false; await loadWorkflow(true); toast('Pesan terpilih ditandai sebagai Hasil Produk.');}catch(e){toast(e.message,true);}};
  ...
  var workflowActivate=document.getElementById('workflowActivate'); if(workflowActivate) workflowActivate.onclick=async function(){var id=workflowCurrentId(); if(!id)return toast('Pilih workflow dulu.',true); try{await api('workflow-activate',{workflow_id:id}); state.workflowLoaded=false; await load(); await loadWorkflow(true); toast('Workflow aktif. Order berbayar untuk produk ini sekarang mengikuti rekaman supplier.');}catch(e){toast(e.message,true);}};
