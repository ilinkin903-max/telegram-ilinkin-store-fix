...
<div class="wrap">
  ...
  <nav class="navTiles" id="navTiles">
    ...
    <button class="tile" data-tab="deepStats"><span class="ico">📈</span>Statistik+</button>
...
email2:password2"></textarea><p class="help">Disembunyikan saat varian aktif karena stok diisi per varian.</p></div>
  ...
  <section id="promos" class="section">
    <div class="panel yellow"><h2 class="sectionTitle">Promo Otomatis</h2><p class="help">Promo otomatis langsung diterapkan saat checkout jika produk, jumlah, dan minimal belanja cocok. Jika user memakai voucher manual, voucher manual diprioritaskan.</p><form id="promoForm" class="form"><div class="row3"><div class="field"><label class="label">Kode Promo</label><input class="input" name="code" placeholder="Contoh: HEMAT10" required></div><div class="field"><label class="label">Nama Promo</label><input class="input" name="name" placeholder="Contoh: Diskon Harian" required></div><div class="field"><label class="label">Status</label><select class="select" name="active"><option value="true">ON</option><option value="false">OFF</option></select></div></div><div class="row3"><div class="field"><label class="label">Tipe Diskon</label><select class="select" name="discount_type"><option value="amount">Nominal Rupiah</option><option value="percent">Persen</option></select></div><div class="field"><label class="label">Nilai Diskon</label><input class="input" name="discount_value" type="number" placeholder="Contoh: 5000 atau 10" required></div><div class="field"><label class="label">Limit Pemakaian</label><input class="input" name="usage_limit" type="number" placeholder="0 = tanpa limit"></div></div><div class="row"><div class="field"><label class="label">Produk Target</label><textarea class="textarea" name="products" placeholder="Kosong/semua = semua produk\nAtau isi kode produk: CANVA1B, NETFLIX"></textarea></div><div class="field"><label class="label">Syarat Promo</label><div class="row"><input class="input" name="min_qty" type="number" placeholder="Min jumlah, contoh 2"><input class="input" name="min_spend" type="number" placeholder="Min belanja, contoh 50000"></div><textarea class="textarea" name="description" placeholder="Catatan promo, contoh: Berlaku untuk pembelian minimal 2 pcs."></textarea></div></div><div class="row"><div class="field"><label class="label">Mulai</label><input class="input" name="start_at" type="datetime-local"></div><div class="field"><label class="label">Berakhir</label><input class="input" name="end_at" type="datetime-local"></div></div><button class="btn lime" type="submit">Simpan Promo</button></form></div>
    <div class="panel"><h2 class="sectionTitle">Daftar Promo</h2><div id="promoList"></div></div>
  </section>

  <section id="deepStats" class="section">
    <div class="panel purple"><h2 class="sectionTitle">Statistik Lebih Dalam</h2><p class="help">Ringkasan performa toko untuk melihat omset, stok kritis, user terbaik, dan jam ramai.</p><div id="deepStatsBox" class="detailGrid"></div></div>
...
(function(){
  ...
  function renderDeepStats(){
    ...
    var box=document.getElementById('deepStatsBox'); if(box){ var rows=[['Omset Hari Ini',rupiah(d.revenue_today)],['Omset Bulan Ini',rupiah(d.revenue_month)],['Total Omset',rupiah(d.revenue_total)],['Rata-rata Order',rupiah(d.average_order_value)],['Total Item Terjual',d.quantity_sold||0],['Conversion Est.',(d.conversion_rate||0)+'%'],['Promo Aktif',d.active_promos||0],['Pending Order',d.pending_orders||0]]; box.innerHTML=rows.map(function(r){return '<div class="detailItem"><b>'+esc(r[0])+'</b><br><span style="font-size:22px">'+esc(r[1])+'</span></div>';}).join(''); }
