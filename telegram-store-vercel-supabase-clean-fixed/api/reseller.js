  ...
  <section id="workflowSettings" class="section">
    ...
    <div class="panel" id="workflowRecorderPanel">
      ...
      <div class="workflowRecorder">
        ...
        <div><h3 style="margin-top:0">Langkah Terekam</h3><div id="workflowStepList" class="workflowSteps"></div><div class="actions" style="margin-top:10px"><button class="btn yellow" id="workflowUndo" type="button">↩️ Hapus Step Terakhir</button><button class="btn lime" id="workflowActivate" type="button">✅ Selesai & Aktifkan</button><button class="btn red" id="workflowDelete" type="button">🗑️ Hapus Workflow</button></div></div>
      </div>
    </div>
...
(function(){
  ...
  var state = { stats:{}, products:[], orders:[], poOrders:[], users:[], vouchers:[], polls:[], settings:{}, analytics:{}, maintenance:{}, backups:[], promos:[], deepStats:{}, license:{}, promoTargets:[], supplierStatus:{}, supplierProducts:[], supplierOrders:[], supplierLoaded:false, workflowStatus:{}, workflows:[], workflowRuns:[], workflowDetail:null, workflowLoaded:false };
  ...
  function renderWorkflow(){
    ...
    var title=document.getElementById('workflowRecorderTitle'); if(title) title.textContent=current?(current.name+' · '+current.product_code+(current.variant_key?' / '+current.variant_key:'')+' → '+current.target_username+(current.active?' · AKTIF':' · MODE REKAM')):'Pilih atau buat workflow untuk mulai merekam.';
  ...
  function renderProducts(){
    ...
    document.getElementById('productList').innerHTML=rows.map(function(p){
      ...
      var vars=visibleVars.map(function(v){
        ...
        var mode=supplierVariant?(workflowVariant?'WORKFLOW BOT':('SUPPLIER · stok '+Math.max(0,Number(v.supplier_stock||0)))):(String(v.delivery_mode||p.delivery_mode||'auto')==='po'?'PO':(variantStock(v).length+' stok'));
      ...
      var availability=isSupplier?(isWorkflow?'WORKFLOW RESELLER':'SUPPLIER OTOMATIS'):(hasSupplierVariants?(hasWorkflowVariants?'VARIAN WORKFLOW/SUPPLIER':(isPo?'PRE-ORDER + SUPPLIER':'AUTO + SUPPLIER')):(isPo?'PRE-ORDER':('STOK '+stockCount(p))));
      ...
      return '<article class="product '+(isPo?'poProduct ':'')+(p.active===false?'productOff':'')+'">'+
        '<div class="productTop">'+productMediaHtml(p)+'<div class="productInfo"><h3>'+esc(p.nama)+'</h3><div class="subtle">'+esc(p.category||'Produk')+' - '+availability+(varsArr.length?' - '+varsArr.length+' varian':'')+'<br><span class="scopeBadge '+(p.display_scope==='marketplace'?'market':'')+'">'+(p.display_scope==='marketplace'?'MARKETPLACE SAJA':'BOT + MARKETPLACE')+'</span>'+(isSupplier?'<br><span class="deliveryModeBadge">'+(isWorkflow?'WORKFLOW RESELLER · BOT SUPPLIER':'SUPPLIER OTOMATIS · PRODSELLER')+'</span>':(hasSupplierVariants?'<br><span class="deliveryModeBadge">'+(hasWorkflowVariants?'MEMILIKI VARIAN WORKFLOW RESELLER':'MEMILIKI VARIAN SUPPLIER · PRODSELLER')+'</span>':(isPo?'<br><span class="deliveryModeBadge">PRE-ORDER · KIRIM MANUAL</span>':'')))+'</div></div><button class="statusToggle '+(p.active===false?'off':'')+'" data-toggle-product="'+esc(p.kode)+'">'+(p.active===false?'OFF':'ON')+'</button></div>'+ 
