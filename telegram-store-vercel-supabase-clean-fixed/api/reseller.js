  ...
  <section id="workflowSettings" class="section">
    ...
    <div class="panel" id="workflowRecorderPanel">
      ...
      <div class="workflowRecorder">
        <div>
          <div class="workflowMessage"><b>Balasan bot supplier terakhir</b><div id="workflowLastMessage"><div class="empty">Belum ada balasan. Ketik /start lalu Kirim Teks.</div></div><div id="workflowMessageButtons" class="workflowButtons"></div></div>
          <div class="field" style="margin-top:12px"><label class="label">Kirim Teks untuk Step Berikutnya</label><textarea class="textarea" id="workflowTextInput" placeholder="/start atau teks biasa. Bisa gunakan {quantity}, {invoice}, {username}, {telegram_id}, {custom_input}"></textarea></div>
          <div class="workflowPlaceholder"><button class="btn small yellow" type="button" data-workflow-insert="/start">/start</button><button class="btn small purple" type="button" data-workflow-insert="{quantity}">{quantity}</button><button class="btn small purple" type="button" data-workflow-insert="{invoice}">{invoice}</button><button class="btn small purple" type="button" data-workflow-insert="{username}">{username}</button></div>
          <div class="actions" style="margin-top:9px"><button class="btn cyan" id="workflowSendText" type="button">✍️ Kirim Teks & Rekam</button><button class="btn yellow" id="workflowRefreshMessage" type="button">🔄 Refresh Balasan</button><button class="btn lime" id="workflowMarkResult" type="button">📦 Balasan Ini = Hasil Produk</button></div>
        </div>
...
(function(){
  ...
  function renderWorkflow(){
    ...
    var snap=current?(current.last_message_snapshot||{}):{}; var msg=document.getElementById('workflowLastMessage');
    if(msg) msg.innerHTML=snap&&snap.id?('<div class="subtle">Message ID '+esc(snap.id)+(snap.edit_date?' · diedit '+esc(snap.edit_date):'')+'</div><pre>'+esc(snap.text||'[Pesan tanpa teks]')+'</pre>'):'<div class="empty">Belum ada balasan. Ketik /start lalu Kirim Teks.</div>';
    var buttons=document.getElementById('workflowMessageButtons');
    if(buttons) buttons.innerHTML=(snap.buttons||[]).map(function(b){ if(b.kind==='url') return '<button class="btn small" type="button" disabled title="Tombol URL tidak dapat dijadikan step">🔗 '+esc(b.text)+'</button>'; return '<button class="btn small purple" type="button" data-workflow-button="'+encodeURIComponent(String(b.text||''))+'">🔘 '+esc(b.text)+'</button>'; }).join('')||'';
    ...
    if(stepList) stepList.innerHTML=steps.map(function(step){var response=step.response_snapshot||{}; return '<div class="workflowStep '+(step.capture_result?'result':'')+'"><b>STEP '+esc(step.step_order)+' · '+(step.action_type==='button'?'🔘 KLIK TOMBOL':'✍️ KIRIM TEKS')+'</b>'+(step.capture_result?' <span class="chip green">HASIL PRODUK</span>':'')+'<br><code>'+esc(step.action_value)+'</code>'+(step.preview_value&&step.preview_value!==step.action_value?'<br><span class="subtle">Saat rekam dikirim: '+esc(step.preview_value)+'</span>':'')+(response.id?'<div class="stepResponse">Balasan: '+esc(response.text||'[tanpa teks]')+'</div>':'<div class="stepResponse">Balasan baru belum terdeteksi.</div>')+'</div>';}).join('')||'<div class="empty">Belum ada step.</div>';
  ...
  document.querySelectorAll('[data-workflow-insert]').forEach(function(btn){btn.onclick=function(){var input=document.getElementById('workflowTextInput'); if(!input)return; var value=btn.dataset.workflowInsert||''; if(value==='/start')input.value='/start'; else input.value=(String(input.value||'')+(input.value?' ':'')+value); input.focus();};});
  var workflowSendText=document.getElementById('workflowSendText'); if(workflowSendText) workflowSendText.onclick=async function(){var id=workflowCurrentId(); var input=document.getElementById('workflowTextInput'); var text=String((input&&input.value)||'').trim(); if(!id)return toast('Pilih workflow dulu.',true); if(!text)return toast('Isi teks yang akan dikirim.',true); workflowSendText.disabled=true; var old=workflowSendText.textContent; workflowSendText.textContent='Mengirim...'; try{await api('workflow-action',{workflow_id:id,action_type:'text',action_value:text}); if(input)input.value=''; state.workflowLoaded=false; await loadWorkflow(true);}catch(e){toast(e.message,true);}finally{workflowSendText.disabled=false;workflowSendText.textContent=old;}};
  var workflowRefreshMessage=document.getElementById('workflowRefreshMessage'); if(workflowRefreshMessage) workflowRefreshMessage.onclick=async function(){var id=workflowCurrentId(); if(!id)return toast('Pilih workflow dulu.',true); workflowRefreshMessage.disabled=true; try{await api('workflow-refresh',{workflow_id:id}); state.workflowLoaded=false; await loadWorkflow(true); toast('Balasan supplier diperbarui.');}catch(e){toast(e.message,true);}finally{workflowRefreshMessage.disabled=false;}};
  var workflowMarkResult=document.getElementById('workflowMarkResult'); if(workflowMarkResult) workflowMarkResult.onclick=async function(){var id=workflowCurrentId(); if(!id)return toast('Pilih workflow dulu.',true); try{await api('workflow-mark-result',{workflow_id:id}); state.workflowLoaded=false; await loadWorkflow(true); toast('Balasan step terakhir ditandai sebagai Hasil Produk.');}catch(e){toast(e.message,true);}};
