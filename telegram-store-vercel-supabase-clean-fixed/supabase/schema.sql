...
begin
  ...
  if v_variant_key <> '' then
    ...
    if lower(coalesce(v_variant->>'stock_mode','separate')) = 'shared' then
      update public.products set stock = v_rest, updated_at = v_now where id = v_product.id;
    end if;
  else
    update public.products
       set stock = v_rest,
...
create table if not exists public.reseller_workflow_steps (
  ...
  recorder_before_snapshots jsonb not null default '[]'::jsonb,
  source_message_snapshot jsonb not null default '{}'::jsonb,
  response_mode text not null default 'wait',
