...
(function(){
  ...
  function editFormHtml(p){
    ...
    var variantSection=hasVar ? '<div class="panel" style="box-shadow:var(--soft);margin:10px 0;background:#f4e7ff"><h3 style="margin:0 0 8px">Edit Data Varian</h3><p class="help">Edit varian di sini hanya untuk nama, harga, kode, grosir, deskripsi, dan SnK. Stok varian diatur dari tombol Stok atau Kelola.</p><input type="hidden" name="variants_text" id="editVariantsText"><textarea class="textarea tall" id="editVariantsMeta" name="variants_meta_text" placeholder="Nama Varian|Harga|Kode|Harga Grosir|Deskripsi Varian|SnK Varian">'+esc(variantMetaToText(p.variants||[]))+'</textarea><p class="ghost">Contoh:<br>1 Bulan|10000|BULAN1|5:9000,10:8000|Deskripsi 1 bulan|SnK 1 bulan</p></div>' : '';
    var baseNoVariant = hasVar ? '<input type="hidden" name="harga" value="'+esc(p.harga||0)+'"><input type="hidden" name="deskripsi" value="'+esc(p.deskripsi||'')+'"><input type="hidden" name="snk" value="'+esc(p.snk||'')+'"><input type="hidden" name="bulk_text" value="'+esc(bulkToText(p.bulk_prices||[]))+'">' : '<div class="row"><div class="field"><label class="label">Harga Satuan</label><input class="input" name="harga" type="number" placeholder="Harga satuan" value="'+esc(p.harga)+'"></div><div class="field"><label class="label">Harga Grosir</label><textarea class="textarea" name="bulk_text" placeholder="5|5000\n10|9000">'+esc(bulkToText(p.bulk_prices||[]))+'</textarea></div></div><div class="row"><div class="field"><label class="label">Deskripsi</label><textarea class="textarea" name="deskripsi" placeholder="Deskripsi produk">'+esc(p.deskripsi||'')+'</textarea></div><div class="field"><label class="label">Syarat & Ketentuan</label><textarea class="textarea" name="snk" placeholder="Syarat & ketentuan produk">'+esc(p.snk||'')+'</textarea></div></div>';
    return '<form id="modalEditForm" class="form"><input type="hidden" name="kode" value="'+esc(p.kode)+'"><div class="row"><div class="field"><label class="label">Nama Produk</label><input class="input" name="nama" placeholder="Nama produk" value="'+esc(p.nama)+'"></div><div class="field"><label class="label">Kode Produk</label><input class="input" name="kode_baru" placeholder="Kode baru" value="'+esc(p.kode)+'"></div></div><div class="row"><div class="field"><label class="label">Kategori</label><input class="input" name="category" placeholder="Kategori" value="'+esc(p.category||'')+'"></div><div class="field"><label class="label">Link Gambar Produk</label><input class="input" name="image_url" placeholder="URL gambar" value="'+esc(p.image_url||'')+'"></div></div>'+baseNoVariant+variantSection+'<button class="btn cyan" type="submit">Simpan Perubahan</button></form>';
  }
  function openEditProduct(code){ var p=findProduct(code); if(!p) return; openModal('Edit Produk - '+p.nama, editFormHtml(p)); document.getElementById('modalEditForm').onsubmit=async function(e){ e.preventDefault(); var d=formData(e.target); d.current_code=d.kode; if(d.kode_baru) d.kode=d.kode_baru; delete d.kode_baru; if((p.variants||[]).length>0){ var meta=document.getElementById('editVariantsMeta'); d.variants_text=mergeVariantMetaText(p, meta?meta.value:''); } delete d.variants_meta_text; await post('edit-product-full',d); closeModal(); }; }
